...
try{ firebase.firestore().settings({ignoreUndefinedProperties:true,experimentalAutoDetectLongPolling:true,useFetchStreams:false}); }catch(_){}
...
(function(){const ACTIVE="is-active";const aAct=WA.classList.contains(ACTIVE),bAct=WB.classList.contains(ACTIVE);if(aAct&&bAct){WB.classList.remove(ACTIVE);}else if(!aAct&&!bAct){WA.classList.add(ACTIVE);}})();
...
/* === AI CHAT label — forced === */
function ensureAIChatLabel(btn){ if(!btn) return; btn.textContent="AI CHAT"; btn.setAttribute("aria-label","AI CHAT"); btn.setAttribute("role","button"); }
...
function bindSelection(refs){
  refs.cards.forEach(card=>{
    ...
    card.addEventListener("click",()=>{ 
      if(READ_ONLY||ANSW) return; vibrateShort();
...
<script>
/* ==== MULTI-DEVICE: lock + heartbeat + live sync ==== */
const sessColl=db.collection("users").doc(user.uid).collection("session_locks");
const sessRef=sessColl.doc(CHAP);
const SESSION_ID=(()=>{const s=crypto&&crypto.getRandomValues?crypto.getRandomValues(new Uint32Array(4)):[Math.random()*1e9,Math.random()*1e9,Math.random()*1e9,Math.random()*1e9];return 's-'+Date.now().toString(36)+'-'+Array.from(s).map(x=>x.toString(36)).join('');})();
let READ_ONLY=false, hbTimer=null, sessionUnsub=null, progUnsub=null;
const STALE_MS=25000, HB_MS=10000;

function ensureSessionBanner(){
  let b=$(".session-banner");
  if(!b){
    b=D.createElement("div"); b.className="session-banner"; b.innerHTML=`<span class="sb-text">Capitolul e deschis activ pe alt dispozitiv. Ești în modul vizualizare.</span><button class="sb-btn sb-take">Preia controlul</button>`;
    D.body.appendChild(b);
    b.querySelector(".sb-take").addEventListener("click",async()=>{ await acquireLock({force:true}); });
  }
  return b;
}
function applyReadOnlyToRefs(refs,ro){
  if(!refs) return;
  const dis=(el,on)=>{ if(!el) return; if(on){ el.setAttribute("aria-disabled","true"); el.classList.add("is-disabled"); el.style.pointerEvents="none"; } else{ el.setAttribute("aria-disabled","false"); el.classList.remove("is-disabled"); el.style.pointerEvents=""; } };
  dis(refs.submitBtn,ro);
  refs.cards.forEach(c=>{ c.style.pointerEvents=ro?"none":""; });
}
function applyReadOnlyUI(ro){
  READ_ONLY=!!ro;
  const ban=ensureSessionBanner();
  ban.classList.toggle("active",READ_ONLY);
  applyReadOnlyToRefs(REFS_A,READ_ONLY);
  applyReadOnlyToRefs(REFS_B,READ_ONLY);
}
async function acquireLock({force=false}={}){
  try{
    const res=await db.runTransaction(async tx=>{
      ...
      const now=Date.now();
      if(!snap.exists){
        tx.set(sessRef,{ownerId:SESSION_ID,chapterSlug:CHAP,ownerAgent:navigator.userAgent||"",ownerSince:firebase.firestore.FieldValue.serverTimestamp(),lastHeartbeat:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});
        return {owned:true};
      }
      const d=snap.data(); const last=d.lastHeartbeat?.toMillis?d.lastHeartbeat.toMillis():0; const stale=!last||((now-last)>STALE_MS);
      if(force||stale||d.ownerId===SESSION_ID){
        tx.set(sessRef,{ownerId:SESSION_ID,ownerAgent:navigator.userAgent||"",lastHeartbeat:firebase.firestore.FieldValue.serverTimestamp(),chapterSlug:CHAP},{merge:true});
        return {owned:true};
      }
      return {owned:false,other:d.ownerId||"?"};
    });
    if(res.owned){
  ...
  }catch(e){ console.warn("acquireLock err:",e); applyReadOnlyUI(true); }
}
function startHeartbeat(){ stopHeartbeat(); if(READ_ONLY) return; hbTimer=setInterval(()=>{ sessRef.set({lastHeartbeat:firebase.firestore.FieldValue.serverTimestamp()},{merge:true}).catch(()=>{}); },HB_MS); }
function stopHeartbeat(){ if(hbTimer){ clearInterval(hbTimer); hbTimer=null; } }
function watchSession(){
  if(sessionUnsub) sessionUnsub();
  sessionUnsub=sessRef.onSnapshot(s=>{
    const d=s.data(); if(!d){ return; }
    const last=d.lastHeartbeat?.toMillis?d.lastHeartbeat.toMillis():0;
    const fresh=last && (Date.now()-last<STALE_MS);
    const iOwn=(d.ownerId===SESSION_ID);
    const shouldRO=!(iOwn)&&fresh;
    applyReadOnlyUI(shouldRO);
    if(iOwn) startHeartbeat(); else stopHeartbeat();
  });
}

/* ==== PROGRES LIVE – fără flicker ==== */
function watchProgress(){
  if(progUnsub) progUnsub();
  progUnsub=progRef.onSnapshot(async snap=>{
    if(!snap.exists) return;
    if(snap.metadata.hasPendingWrites) return; // evităm eco local
    const d=snap.data();
    const idx=(typeof d.currentIndex==="number")?d.currentIndex:CUR;
    const fin=!!d.finished;
    const changedIdx=idx!==CUR;
    CUR=idx; FIN=fin;
    if(FIN){
      await showFinal();
      return;
    }
    if(changedIdx && !WRMODE){
...
async function startWrongModeSafe(){
  if(READ_ONLY) { toast("Ești în modul vizualizare. Apasă „Preia controlul” pentru a continua.",1400); return; }
...
/* ==== HANDLERE BUTOANE (submit/verify/next) ==== */
...
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    if(SEL.length===0){
      toast("Selectează un răspuns!",1000); playOne(alertSound);
...
D.addEventListener("click",async e=>{
  ...
  if(e.target.closest(".reset-chapter-btn")){
    if(READ_ONLY){ toast("Ești în modul vizualizare. Nu poți reseta de aici.",1200); return; }
...
D.addEventListener("keydown",e=>{
  const act=activeWrap(), refs=refsOf(act);
  if(e.key==="Enter"){
    if(READ_ONLY){ toast("Ești în modul vizualizare. Apasă „Preia controlul”.",1200); return; }
    const btn=refs?.submitBtn; if(btn){ e.preventDefault(); btn.click(); }
...
/* ==== INITIALIZARE: lock -> fetch progres -> render -> subscribe ==== */
async function initialLoad(){
  ensureSessionBanner(); // creează UI
  watchSession();        // ne abonăm pentru a schimba RO/owner live
  await acquireLock({force:false});

  // Citește progresul o dată (fără flicker), apoi pornește watcherul
  try{
    const snap=await progRef.get();
    if(snap.exists){
      const d=snap.data();
      if(typeof d.currentIndex==="number") CUR=d.currentIndex;
      if(d.finished) FIN=true;
    }
  }catch(_){}
  if(FIN){
    await showFinal();
  ...
  watchProgress(); // de aici înainte mergem pe snapshot live
}

initialLoad().catch(e=>{ console.error("Initial load failed:",e); revealNow(); });

/* ==== CLEANUP (TTL, fără clear lock explicit – lăsăm să expire) ==== */
