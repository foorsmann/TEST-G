...
/* ==== MOBILE + DESKTOP: helpers pentru oglinzirea stărilor butoanelor ==== */
const refs={
...
function setSubmitState(_ignoredBtn,state){
...
/* HYDRATE + acordion + MOBILE bindings */
...
function hydrateExistingDom(){
  ...
  if (refs.buttonsWrapMobile) {
    ...
    // menținem oglindirea clasei single-btn-state (desktop -> mobil)
    new MutationObserver(mirrorSingleState).observe(refs.buttonsWrap,{attributes:true,attributeFilter:["class"]});
    mirrorSingleState(); // init sync
  }
...
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  // Single-button state la început (oglindit pe mobil)
  ...
  try{
    ...
    if(snap.exists){
      ...
      if(allowRestore && selSaved.length>0){
        ...
        refs.feedbackWrap.style.setProperty("display","flex","important"); refs.explainBtn?.style.setProperty("display","flex","important"); refs.aiBtn?.style.setProperty("display","flex","important"); ensureAIChatLabel(true);
...
/***** SUBMIT / NEXT / FINISH – acum pentru desktop + mobil *****/
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    if(SEL.length===0){
      ...
      refs.buttonsWrap.classList.add("single-btn-state");
      mirrorSingleState();
      getSubmitBtns().forEach(b=>{ if(b) b.style.gridColumn="1 / 2"; });
      return;
    }
    ...
    // arătăm butoanele auxiliare și pe desktop, și pe mobil (prin ieșirea din single)
    refs.buttonsWrap.classList.remove("single-btn-state");
    mirrorSingleState();

    // text/aria pentru AI pe ambele
    ensureAIChatLabel(true);
    if(refs.aiBtnMobile){
      const t=(refs.aiBtnMobile.textContent||"").trim(); if(!t) refs.aiBtnMobile.textContent="AI CHAT";
      refs.aiBtnMobile.setAttribute("aria-label","AI CHAT");
...
/***** QoL & Safety *****/
function getPreferredSubmitButton(){
  if(isVisible(refs.submitBtnMobile)) return refs.submitBtnMobile;
  return refs.submitBtn || refs.submitBtnMobile || null;
}
D.addEventListener("keydown",e=>{
  if(e.key==="Enter"){
    const btn=getPreferredSubmitButton();
