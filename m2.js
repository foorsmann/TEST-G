...
/* ===== Rămâne neschimbat: setup Firebase / contexte ===== */
...
/* ===== Stări buton submit (cu suport desktop/mobile) ===== */
...
function setSubmitStateBoth(state){
  /* Desktop – poate atinge layoutul (gridColumn) */
  setSubmitState(refs.submitBtn, state, {affectLayout:true});
  /* Mobile – nu atingem layoutul; Webflow decide pozițiile */
  setSubmitState(refs.submitBtnMobile, state, {affectLayout:false});
}

/* Doar desktop-ul primește clasa single-btn-state; mobilul rămâne 100% Webflow */
...
/* ===== Option helpers (nerevizuite semantic) ===== */
...
function scrollContainerToAnchor(target,container,{behavior="smooth",offset=0,delay=0}={}){
  ...
  /* BUGFIX: folosește `dur` (nu `d`) în easing */
...
function hydrateExistingDom(){
  ...
  /* toggling explicații – sincron desktop + mobile, fără a schimba layoutul mobil */
  ...
  /* AI Chat – deschide pop-ul (desktop + mobile) */
  const onAiClick=()=>{
    vibrateShort();
    const v=refs.videoWrap?.querySelector("video");
...
function setButtonsTransitionLock(on){
  ...
  /* pe mobil nu atingem display-ul sau grid-ul, doar pointer-events */
...
/* ===== Video block (neschimbat semantic) ===== */
...
/* === restaurare stare “verificată” (și în wrong-mode doar pentru sesiunea curentă) === */
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  /* ascundem explicația + AI (desktop + mobil) până după verificare */
  ...
  /* restaurare din Firestore (cu condiția de sesiune în wrong-mode) */
  try{
...
/* --- Patch post-definire pentru a repara eventuale suprascrieri accidentale ale style.setProperty (ex. pe aiBtnMobile) --- */
function __repairStyleSetProperty(el){
  try{
    if(el && el.style && typeof el.style.setProperty !== "function"){
      delete el.style.setProperty; // readuce metoda din prototip
    }
  }catch(_){}
}
/* Wrap pentru updateInPlace: după fiecare update, reparam setProperty dacă a fost corupt */
...
/* reparăm din start orice .style.setProperty afectat accidental */
__repairStyleSetProperty(refs.aiBtnMobile);
...
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    ...
    /* vizibilitate explicație + AI – desktop + mobil (layout mobil rămâne din Webflow) */
    refs.feedbackWrap.style.setProperty("display","flex","important");
    if(refs.explainBtn) refs.explainBtn.style.setProperty("display","flex","important");
    if(refs.explainBtnMobile) refs.explainBtnMobile.style.setProperty("display","flex","important");
    if(refs.aiBtn) refs.aiBtn.style.setProperty("display","flex","important");
