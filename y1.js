...
const pad2=n=>String(n).padStart(2,"0"),HAS_PLYR=!!(W&&W.Plyr);
...
/* Auth: lăsăm login-ul la intrare (stabil) */
...
/* ===== AB (Double Buffer) Setup ===== */
const AB={active:'A',A:null,B:null};
function setupAB(){
  if(!QWRAP) return;
  if(QWRAP.closest('.question-wrapper-ab')){
    // deja în container — mapăm A/B din DOM dacă există
    AB.A=QSEC.querySelector('.question-wrapper-A')||QWRAP;
    AB.B=QSEC.querySelector('.question-wrapper-B')||QSEC.querySelectorAll('.question-wrapper')[1]||null;
    AB.active=(AB.A&&AB.A.classList.contains('visible'))?'A':'A';
    return;
  }
  const container=D.createElement('div'); container.className='question-wrapper-ab';
  const parent=QWRAP.parentElement;
  QWRAP.classList.add('q-fade','question-wrapper-A'); QWRAP.dataset.ab='A';
  const clone=QWRAP.cloneNode(true); clone.classList.add('q-fade','question-wrapper-B'); clone.dataset.ab='B';
  // curățăm selecții/clase legate de stări la bufferul B
  clone.classList.remove('visible','is-preparing');
  clone.querySelectorAll('.selected, .correct, .wrong, .wrong-card').forEach(el=>el.classList.remove('selected','correct','wrong','wrong-card'));
  // montăm containerul
  parent.insertBefore(container,QWRAP);
  container.appendChild(QWRAP);
  container.appendChild(clone);
  // A este vizibil inițial
  QWRAP.classList.add('visible');
  AB.A=QWRAP; AB.B=clone; AB.active='A';
}
setupAB();

/* Recalc înălțime în timpul swap-ului */
function fitABHeight(){
  const cont=AB.A?.parentElement?.classList.contains('question-wrapper-ab')?AB.A.parentElement:null;
  if(!cont) return;
  const aH=AB.A?.offsetHeight||0, bH=AB.B?.offsetHeight||0;
  const H=Math.max(aH,bH);
  if(H>0){ cont.style.minHeight=H+'px'; }
}
function releaseABHeight(){
  const cont=AB.A?.parentElement?.classList.contains('question-wrapper-ab')?AB.A.parentElement:null;
...
/* ===== Explicații: UI sincron desktop + mobil ===== */
...
/* ===== HYDRATE DOM pentru un wrapper dat (A sau B) ===== */
...
/* Hydrate pentru wrapperul “scope” (A sau B) și legare evenimente locale */
function hydrateExistingDom(scope){
  const root=scope || (QSEC||D);
  const need=sel=>root.querySelector(sel);

  refs.qEl=root.closest('.questions-collection-list-item')||root;
  refs.qWrap=need(".question-wrapper")||refs.qEl;
  refs.anchorTop=(QSEC||D).querySelector(".explanation-anchor-top")||null;

  refs.qText=need(".question-text"); if(!refs.qText) return false;
  refs.progress=(QSEC||D).querySelector(".progress-counter"); if(!refs.progress) return false;
  ...
  refs.cards=a(".option-card",refs.optWrap); if(!refs.cards.length) return false;
  ...
  /* desktop */
  refs.buttonsWrap=(QSEC||D).querySelector(".submit-next-buttons"); if(!refs.buttonsWrap) return false;
  ...
  /* mobil (layout controlat de Webflow – poate fi în afara wrapperului) */
  refs.buttonsWrapMobile=(QSEC||D).querySelector(".submit-next-buttons-mobile")||null;
  ...
  refs.feedbackWrap=(QSEC||D).querySelector(".feedback-wrapper")||need(".feedback-wrapper"); if(!refs.feedbackWrap) return false;
  ...
  /* select/deselect opțiuni — doar pe wrapperul activ curent */
  ...
  /* accordion toggling și pressed feedback */
  ...
  /* Explain toggle — sincron desktop + mobil */
...
/* === restaurare stare “verificată” în wrapperul activ === */
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  const container=refs.qWrap||D.body;
  ...
  if(QSEC) QSEC.style.setProperty("display","flex","important");
  refs.qWrap?.classList.add("visible");
  ...
  /* restaurare din Firestore (condiționată de sesiune la wrong-mode) */
  try{
    ...
    if(snap.exists){
      ...
      if(allowRestore && selSaved.length>0){
        ...
        if(refs.aiBtnMobile) refs.aiBtnMobile.style.setProperty="display:flex";
        applyAIChatText();
...
/* --- Repara eventuale suprascrieri ale style.setProperty (defensive) --- */
...
const __origUpdateInPlace = typeof updateInPlace==="function" ? updateInPlace : null;
if(__origUpdateInPlace){
  window.updateInPlace = async function(idx, opts){
    const out = await __origUpdateInPlace(idx, opts);
...
/* Leagă DOM pe wrapperul activ (A) */
if(!hydrateExistingDom(AB.A)){ alert("Structura nu e completă. Verifică clasele HTML."); D.body.classList.remove("page-loading"); return; }
__repairStyleSetProperty(refs.aiBtnMobile);
...
/* Helper pentru swap A/B: redă în bufferul inactiv, apoi crossfade */
async function renderIntoInactiveAndSwap(nextIdx, opts){
  const activeWrap = (AB.active==='A'?AB.A:AB.B);
  const inactiveWrap = (AB.active==='A'?AB.B:AB.A);
  if(!inactiveWrap){ /* fallback in-place */ await updateInPlace(nextIdx, opts); return; }

  // Hidratează pe bufferul inactiv, redă acolo conținutul următor
  if(!hydrateExistingDom(inactiveWrap)){ await updateInPlace(nextIdx, opts); return; }
  await updateInPlace(nextIdx, opts);

  // Fixăm înălțimea containerului pentru a evita reflow în timpul crossfade
  fitABHeight();

  // Crossfade
  activeWrap.classList.remove('visible'); inactiveWrap.classList.add('visible');
  (inactiveWrap===AB.A)?AB.active='A':AB.active='B';

  // Religare refs pe noul activ
  hydrateExistingDom(inactiveWrap);
  setButtonsTransitionLock(false);

  // Eliberăm înălțimea la final de frame
  setTimeout(releaseABHeight,260);
}
...
/* === Index activ curent (normal vs wrong-mode) === */
const getActiveIndex=()=> WRMODE ? WRONG[WRP] : CUR;

/* === SUBMIT / NEXT / FINISH — desktop + mobil (cu A/B swap) === */
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isNext && ANSW){
    ...
    if(WRMODE){
      ...
      pruneBUF(); SEL=[]; ANSW=false;
      await renderIntoInactiveAndSwap(WRONG[WRP],{resetSelections:true});
      return;
    }else{
      if(CUR<TOTAL){
        CUR++; try{ await progRef.set({currentIndex:CUR},{merge:true}); }catch(_){}
        await prefetchAhead(CUR); pruneBUF(); SEL=[]; ANSW=false;
        await renderIntoInactiveAndSwap(CUR,{resetSelections:false});
      }else{
        try{ await progRef.set({finished:true},{merge:true}); }catch(_){}
        await showFinal();
      }
...
/* === WRONG MODE — listă + start cu persistență (cu prefetch & A/B) === */
...
async function startWrongModeSafe(){
  ...
  const activeWrap=(AB.active==='A'?AB.A:AB.B)||QWRAP||D.body;

  if(activeWrap){
    const h=activeWrap.offsetHeight;
    activeWrap.classList.add("no-transition","is-preparing");
    if(h) activeWrap.style.minHeight=h+"px";
  }
  ...
  try{
    ...
    if(!list.length){
      ...
      FINAL?.style.setProperty("display","flex","important");
      if(activeWrap){ activeWrap.classList.remove("no-transition","is-preparing"); activeWrap.style.minHeight=""; }
      D.body.classList.remove("page-loading");
...
/* === Delegări ecran final (Wrong-Mode / Next cap / Reset capitol) === */
...
/* === QoL & Safety (Enter = Submit, Esc = închide pop-upuri) === */
...
/* Failsafe final (scoate anti-flicker oricum) */
D.body.classList.remove("page-loading");
blurActive();
