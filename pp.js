...
/* =========================
   REFS — IMPORTANT: FĂRĂ FALLBACK GLOBAL pe elementele sensibile ca să nu atingem wrapperul activ din greșeală în timpul render-ului bufferului (fix glitch)
   ========================= */
function collectRefs(wrapper){
  ...
  const refs={
    ...
    qWrap:wrapper,
    /* ❗ fără fallback global pt anchorTop/anchor */
    anchorTop:need(".explanation-anchor-top"),
    qText:need(".question-text"),
    /* progress poate rămâne cu fallback dacă e global în layout */
    ...
    /* ❗ Fără fallback global pe video/accordion/feedback/buttons – acestea au efecte vizuale și trebuie strict scoped */
    videoWrap:need(".explanation-video-wrapper"),
    acc:need(".custom-accordion"),
    ...
    anchor:need(".explanation-anchor")
  };
...
/* ✅ FIX #3: Forțează eticheta butonului AI, indiferent de ce text vine din Webflow */
...
/* ✅ FIX #2: aplică/șterge starea NO-IMAGE doar pe WRAPPER-ul curent, nu pe containerul părinte comun,
   ca să nu ascundă imaginea întrebării vizibile înainte de flip. */
function applyNoImageState(refs,on){
  if(!refs.optWrap || !refs.qWrap) return;
  const target=refs.qWrap; /* doar wrapper-ul A/B */
  if(on){
...
function ensureAIPop(){
  ...
  /* Helpers pentru flip și referințe */
  ...
  async function flipToIndex(idx,{resetSelections=false}={}){
    ...
    const act=activeWrap();
    /* schimbă activul după ce bufferul e complet randat */
  ...
  async function showFinal(){
    ...
    /* bara + procent deja au helper-ele încărcate în Partea 1 */
    ...
    /* pregătim WRONG_PREPARED pt modul „refă greșitele” */
    try{
  ...
  /* buton principal (VERIFY / CONTINUA / FINALIZEAZA) */
  QSEC?.addEventListener("click",async(e)=>{
    ...
    if(isVerify){
      ...
      ensureAIChatLabel(refs.aiBtn); /* asigură „AI CHAT” */
      refs.buttonsWrap?.classList.remove("single-btn-state");
      const lastWrong=WRMODE&&(WRP===WRONG.length-1),lastNormal=!WRMODE&&(CUR===TOTAL);
      setSubmitState(refs.submitBtn,(lastWrong||lastNormal)?"finish":"next");
      requestAnimationFrame(()=>{
        const scroller=refs.root?.querySelector(".question-wrapper-inner")||getScrollParent(refs.root);
        const anchor=refs.root.querySelector(".explanation-anchor");
        if(anchor){
  ...
  /* dacă FIN era deja setat, arătăm finalul */
