...
// === SINGLE ACTIVE LOCK pentru acest capitol ===
...
/* nou: începutul sesiunii wrong-mode pentru restaurare condiționată */
let WRONG_STARTED_AT_MS=0;
/* nou: cheie localStorage pentru wrong-mode */
...
async function persistWrongMode({active,list,pos}){
  try{
    const payload={
      active:!!active,
      list:Array.isArray(list)?list.map(v=>+v).filter(Number.isFinite):[],
      pos:Math.max(0,Math.min((Array.isArray(list)?list.length-1:0),(+pos||0))),
      startedAtMs: WRONG_STARTED_AT_MS||Date.now()
    };
...
/* === MOBILE MIRROR HELPERS === */
function isVisible(el){
  if(!el) return false;
  const s=getComputedStyle(el);
  return s.display!=="none" && s.visibility!=="hidden" && el.offsetParent!==null;
}
const BTN_OBS = { attached:false, observer:null };
function getSubmitBtns(){
  return [refs.submitBtn, refs.submitBtnMobile].filter(Boolean);
}
function getAllWrappers(){
  return [refs.buttonsWrap, refs.buttonsWrapMobile].filter(Boolean);
}
function mirrorSingleState(){
  if(!refs.buttonsWrap || !refs.buttonsWrapMobile) return;
  const on = refs.buttonsWrap.classList.contains("single-btn-state");
  refs.buttonsWrapMobile.classList.toggle("single-btn-state", on);
}
function attachButtonsObserver(){
  if(BTN_OBS.attached || !refs.buttonsWrap) return;
  BTN_OBS.observer = new MutationObserver(()=>mirrorSingleState());
  BTN_OBS.observer.observe(refs.buttonsWrap,{attributes:true,attributeFilter:["class"]});
  BTN_OBS.attached = true;
}

function setSubmitState(btn,state){
  // aplicăm pe ambele butoane submit (desktop + mobil), păstrăm semnătura pentru compatibilitate
  const btns = getSubmitBtns();
  btns.forEach(b=>{
    if(!b) return;
    b.classList.remove("is-verify","is-next","is-finish","finish-chapter-btn","_preload-full");
    switch(state){
      case "verify":
        b.textContent="VERIFICA";
        b.classList.add("is-verify","_preload-full");
        b.style.gridColumn="1 / 2";
        break;
      case "next": {
        const last = WRMODE ? (WRP===WRONG.length-1) : (CUR===TOTAL);
        b.textContent = last ? "FINALIZEAZA" : "CONTINUA";
        if(last) b.classList.add("is-finish","finish-chapter-btn"); else b.classList.add("is-next");
        b.style.gridColumn="2 / 3";
        break;
      }
      case "finish":
        b.textContent="FINALIZEAZA";
        b.classList.add("is-finish","finish-chapter-btn");
        b.style.gridColumn="2 / 3";
        break;
    }
  });
...
function setButtonsTransitionLock(on){
  ...
  wraps.forEach(w=>{
    if(!w) return;
    if(on) w.style.setProperty("pointer-events","none","important");
    else w.style.removeProperty("pointer-events");
  });
}
...
/* FINAL SCREEN + PREP LISTA WRONG (prefetch înainte de click) */
...
function hydrateExistingDom(){
  ...
  if (refs.buttonsWrapMobile) {
    refs.submitBtnMobile   = refs.buttonsWrapMobile.querySelector(".submit-btn-mobile") || null;
    refs.explainBtnMobile  = refs.buttonsWrapMobile.querySelector(".explanation-btn-mobile") || null;
    refs.aiBtnMobile       = refs.buttonsWrapMobile.querySelector(".ask-ai-btn-mobile") || null;

    // Explicație – identic cu desktop
    if (refs.explainBtnMobile && !refs.explainBtnMobile.dataset.boundExp) {
      refs.explainBtnMobile.addEventListener("click", ()=>{
        vibrateShort();
        const opening = (refs.explainBtnMobile && refs.explainBtnMobile.innerHTML.indexOf("<svg")===-1);
        ...
        if(opening){
          ...
          refs.explainBtnMobile.innerHTML=`<svg width="15" height="15" viewBox="0 0 32 32"><path d="M16 10l10 10H6z" fill="#fff"/></svg>`;
          ...
          refs.explainBtnMobile.textContent="EXPLICATIE";
    ...
    // AI – identic cu desktop
    if (refs.aiBtnMobile && !refs.aiBtnMobile.dataset.boundAi) {
      const ensureMobileAiLabel = ()=>{
        const t=(refs.aiBtnMobile.textContent||"").trim(); if(!t) refs.aiBtnMobile.textContent="AI CHAT";
        refs.aiBtnMobile.setAttribute("aria-label","AI CHAT");
        refs.aiBtnMobile.setAttribute("role","button");
      };
      ensureMobileAiLabel();
      refs.aiBtnMobile.addEventListener("click", ()=>{
        vibrateShort();
        const v=refs.videoWrap?.querySelector("video");
  ...
  // atașăm observer-ul pentru sync de stare între desktop și mobil
  attachButtonsObserver();
  mirrorSingleState();
...
/* === restaurare stare “verificată” și în wrong-mode, dar doar pentru încercările din sesiunea curentă === */
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  // ascundem ambele seturi Explicatie/AI până se verifică
  refs.explainBtn&&(refs.explainBtn.style.display="none");
  refs.aiBtn&&(refs.aiBtn.style.display="none");
  refs.explainBtnMobile&&(refs.explainBtnMobile.style.display="none");
  refs.aiBtnMobile&&(refs.aiBtnMobile.style.display="none");
  ...
  refs.explainBtnMobile&&(refs.explainBtnMobile.textContent="EXPLICATIE");
  ...
  SEL=[]; ANSW=false; clearLetterStates(refs.qEl); clearAllSelected(refs.qEl); clearWrongCards(refs.qEl);

  refs.buttonsWrap.classList.add("single-btn-state");
  mirrorSingleState();                 // sync cu mobil
  ...
  /* restaurare din Firestore dacă:
     - suntem în test normal (ca înainte), SAU
     - suntem în wrong-mode și răspunsul a fost (re)verificat în sesiunea curentă (answeredAtMs >= WRONG_STARTED_AT_MS)
  */
  try{
    ...
    if(snap.exists){
      ...
      if(allowRestore && selSaved.length>0){
        ...
        // arătăm butoanele explicatie/AI pe ambele seturi
        refs.explainBtn?.style.setProperty("display","flex","important");
        refs.aiBtn?.style.setProperty("display","flex","important");
        refs.explainBtnMobile?.style.setProperty("display","flex","important");
        refs.aiBtnMobile?.style.setProperty("display","flex","important");
        ensureAIChatLabel(true);

        refs.buttonsWrap.classList.remove("single-btn-state");
        mirrorSingleState();
        const lastWrong=WRMODE&&(WRP===WRONG.length-1),lastNormal=!WRMODE&&(CUR===TOTAL); setSubmitState(refs.submitBtn,(lastWrong||lastNormal)?"finish":"next");
      }
    }
...
/* === INIT cu reluare wrong-mode dacă era activ === */
...
if(!hydrateExistingDom()){ alert("Structura nu e completă. Verifică clasele HTML."); D.body.classList.remove("page-loading"); return; }
...
/* AI pop-up (același comportament ca înainte) */
let AIPop=null;
...
/***** SUBMIT / NEXT / FINISH – cu answeredAtMs + suport mobile *****/
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    if(SEL.length===0){
      ...
      playOne(alertSound);
      // forțăm starea de single button pe ambele seturi
      refs.buttonsWrap.classList.add("single-btn-state");
      mirrorSingleState();
      setSubmitState(refs.submitBtn,"verify");
      return;
...
/***** ENTER tastează butonul vizibil (desktop sau mobil) *****/
D.addEventListener("keydown",e=>{
  if(e.key==="Enter"){
    const btn = isVisible(refs.submitBtnMobile) ? refs.submitBtnMobile : refs.submitBtn;
