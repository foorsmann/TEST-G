...
/* ============================================================
   BLOCLU 2: CONTEXT APP + HELPERI UI
   - „must-fix” #2: restaurare fidelă a stării „verificate” după refresh
   - Micro-îmbunătățiri pentru stabilitate și fără flicker
   ============================================================ */
...
/* Login rămâne neschimbat pentru stabilitate */
...
/* === SANITIZARE — folosește unicul sanitizator global === */
...
/* Segmente bară finală (verde/roșu + gap) */
...
/* Prefetch în batch-uri de 10 (Firestore „in” limit) */
...
/* Prefetch înainte (N+1; pe 4G și N+2) */
...
/* Prune BUF (fără .forEach—evităm bugul „Unexpected identifier”) */
function pruneBUF(){
  const keep=new Set();
  if(!WRMODE){
...
function hydrateExistingDom(){
  ...
  /* accordion: feedback + toggle (cu micro-stabilitate) */
...
(function(){
  const o=document.querySelector(".chapter-progress-bar-outer"); if(!o) return;
