...
/* ====== Multi-device ownership (lock + heartbeat) ====== */
const CLIENT_ID=(()=>{try{const k="clientId";let v=localStorage.getItem(k);if(!v){v=(crypto.randomUUID?.()||("cid_"+Date.now()+"_"+Math.random().toString(36).slice(2)));localStorage.setItem(k,v);}return v}catch(_){return "cid_"+Date.now()+"_"+Math.random().toString(36).slice(2)}})();
let IS_OWNER=false, VIEW_ONLY=false, HEARTBEAT=null, progUnsub=null, FIRST_SNAP=true, currentShownIndex=null;
const LOCK_TTL=30000, HEARTBEAT_MS=10000;

function ensureViewOnlyBar(){
  let bar=$(".quiz-view-only");
  if(!bar){
    bar=D.createElement("div"); bar.className="quiz-view-only";
    const msg=D.createElement("div"); msg.className="msg"; msg.textContent="Chestionarul e activ pe alt dispozitiv. Poți vedea conținutul, dar nu poți interacționa.";
    const btn=D.createElement("button"); btn.className="takeover"; btn.textContent="Preia controlul";
    btn.addEventListener("click",()=>claimLock(true));
    bar.append(msg,btn);
    (QSEC||D.body).prepend(bar);
  }
  return bar;
}
function setOwnerState(on){
  IS_OWNER=!!on; VIEW_ONLY=!on;
  const bar=ensureViewOnlyBar();
  if(on){ bar.classList.remove("show"); QSEC?.classList.remove("view-only"); }
  else{ bar.classList.add("show"); QSEC?.classList.add("view-only"); }
}
function startHeartbeat(){ if(HEARTBEAT) return; HEARTBEAT=setInterval(()=>{ if(IS_OWNER) progRef.set({activeUpdatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true}); },HEARTBEAT_MS); }
function stopHeartbeat(){ if(HEARTBEAT){ clearInterval(HEARTBEAT); HEARTBEAT=null; } }
async function claimLock(force=false){
  try{
    await db.runTransaction(async tx=>{
      const snap=await tx.get(progRef); const d=snap.exists?snap.data():{};
      const last=d.activeUpdatedAt, holder=d.activeSessionId;
      const now=firebase.firestore.Timestamp.now();
      const expired=!last || ((now.toMillis()-(last.toMillis?.()||0))>LOCK_TTL);
      if(force||!holder||holder===CLIENT_ID||expired){
        tx.set(progRef,{activeSessionId:CLIENT_ID,activeUpdatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});
        setOwnerState(true);
      }else{
        setOwnerState(holder===CLIENT_ID);
      }
    });
    IS_OWNER?startHeartbeat():stopHeartbeat();
  }catch(e){ console.warn("claimLock fail:",e); }
}
function releaseLock(){ try{ if(IS_OWNER) progRef.set({activeSessionId:firebase.firestore.FieldValue.delete()},{merge:true}); }catch(_){}
  stopHeartbeat(); setOwnerState(false);
}
document.addEventListener("visibilitychange",()=>{ if(document.visibilityState==="visible") claimLock(false); });
window.addEventListener("focus",()=>claimLock(false));
window.addEventListener("beforeunload",()=>releaseLock());

/* ====== BroadcastChannel (multi-tab local) ====== */
try{
  const bc=new BroadcastChannel("quiz-control");
  bc.postMessage({type:"hello",cid:CLIENT_ID,chap:CHAP,ts:Date.now()});
  bc.onmessage=(ev)=>{const m=ev?.data||{}; if(m.type==="claim"&&m.chap===CHAP&&m.cid!==CLIENT_ID){ setOwnerState(false); }};
  const oldClaimLock=claimLock;
  claimLock=async function(force=false){ await oldClaimLock(force); try{ if(IS_OWNER) bc.postMessage({type:"claim",cid:CLIENT_ID,chap:CHAP,ts:Date.now()}); }catch(_){} };
}catch(_){}
...
function scrollContainerToAnchor(target,container,{behavior="smooth",offset=0,delay=0}={}){
  ...
  const step=ts=>{ if(!st) st=ts; const p=Math.min((ts-st)/d,1),y=Math.round(start+dist*easeInOut(p)); isWin?window.scrollTo(0,y):c.scrollTop=y; if(p<1) requestAnimationFrame(step); };
  delay?setTimeout(()=>requestAnimationFrame(step),delay):requestAnimationFrame(step);
}

/* ====== Collect refs din wrapper ====== */
...
/* Eticheta AI CHAT – forțată local */
...
function bindSelection(refs){
  refs.cards.forEach(card=>{
    ...
    card.addEventListener("click",()=>{ 
      if(ANSW||VIEW_ONLY) return; vibrateShort();
...
/* no-image LOCAL pe wrapper curent */
function applyNoImageState(refs,on){
  if(!refs.optWrap || !refs.root) return;
  if(on){
    refs.root.classList.add("no-image-local");
    refs.optWrap.style.minHeight="40vh";
    if(refs.imgWrap) refs.imgWrap.style.display="none";
  }else{
    refs.root.classList.remove("no-image-local");
...
/* Stop media la schimbare întrebare */
...
/* Randare întrebare într-un wrapper */
...
/* ====== A/B init + flip ====== */
function hydrateWrapper(wrapper){ const refs=collectRefs(wrapper); wrapper.classList.add("question-wrapper","q-fade"); return refs; }
...
function updateRefsAfterFlip(){ const act=activeWrap(); REFS=(act===WA)?(REFS_A=collectRefs(WA)):(REFS_B=collectRefs(WB)); }

async function flipToIndex(idx,{resetSelections=false}={}){
  const act=activeWrap(), buf=bufferWrap();
  const actRefs=refsOf(act), bufRefs=refsOf(buf);
  stopActiveMedia(actRefs);
  ...
  requestAnimationFrame(()=>{ act.classList.remove("no-transition","is-preparing"); act.style.minHeight=""; });
  updateRefsAfterFlip();
}
...
async function showFinal(){
  ...
  try{
    ...
    answersSnap.forEach(doc=>{
      ...
      if(sel&&sel!==cor){
        let idx=d.index;
        if(typeof idx!=="number"){ const n=parseInt(idx,10); if(Number.isFinite(n)) idx=n; }
    ...
    if(resetWrongBtn) resetWrongBtn.style.display=WRONG_PREPARED.length?"flex":"none";
...
async function startWrongModeSafe(){
  ...
  try{
    ...
    if(!list.length){
      alert("Nu ai nicio intrebare gresita de refăcut la acest capitol!");
      FINAL?.style.setProperty("display","flex","important");
      if(QSEC) QSEC.style.setProperty("display","none","important");
      return;
    }
...
/* ====== HANDLERE & SNAPSHOT (multi-device, fără flicker) ====== */

/* index activ curent (în modul normal sau wrong-mode) */
const getActiveIndex=()=> WRMODE?WRONG[WRP]:CUR;

/* CLICK pe zona de întrebări (submit/next/finish) */
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(VIEW_ONLY){
    toast("Ești în modul vizualizare. Apasă „Preia controlul” ca să continui.",1400);
    ensureViewOnlyBar(); return;
  }
  ...
  const act=activeWrap(), refs=refsOf(act);
  ...
  if(isNext && ANSW){
    if(WRMODE){
      ...
      if(CUR<TOTAL){
        try{ await progRef.set({currentIndex:CUR+1,finished:false},{merge:true}); }catch(_){}
        prefetchAhead(CUR+1); /* lăsăm onSnapshot să randeze, ca să nu apară flash vizual */
      }else{
...
/* ALTE CLICK-URI GLOBALE (reset capitol, next capitol etc.) */
D.addEventListener("click",async e=>{
  ...
  if(e.target.closest(".confirm-reset-chapter")){
    if(VIEW_ONLY){ toast("Nu poți reseta în modul vizualizare.",1100); return; }
    try{
      await progRef.delete();
      const qs=await ansCollection.where("chapterSlug","==",CHAP).get();
      const batch=firebase.firestore().batch(); qs.forEach(doc=>batch.delete(doc.ref)); await batch.commit();
      location.reload();
    }catch(_){
...
/* TASTATURĂ (Enter = submit, Esc = închide pop-up/loader) */
D.addEventListener("keydown",e=>{
  const act=activeWrap(), refs=refsOf(act);
  if(e.key==="Enter"){ const btn=refs?.submitBtn; if(btn){ e.preventDefault();
    if(VIEW_ONLY){ toast("Ești în modul vizualizare. Apasă „Preia controlul”.",1200); return; }
    btn.click(); } }
...
/* ====== SNAPSHOT pe progres – sursa de adevăr (elimină flicker, multi-device corect) ====== */
await claimLock(false); /* încearcă să devii owner sau intră în view-only */
progUnsub = progRef.onSnapshot(async snap=>{
  const exists=snap.exists, d=exists?snap.data():{};
  /* actualizează ownership după lock/heartbeat altui device */
  try{
    const holder=d.activeSessionId, last=d.activeUpdatedAt;
    const now=firebase.firestore.Timestamp.now();
    const expired=!last || ((now.toMillis()-(last.toMillis?.()||0))>LOCK_TTL);
    const iOwn = !holder || holder===CLIENT_ID || expired;
    setOwnerState(iOwn); iOwn?startHeartbeat():stopHeartbeat();
  }catch(_){}

  /* dacă nu există doc, nu forțăm aici; prima interacțiune îl va crea */
  const idxRaw=d.currentIndex, finished=!!d.finished;
  const idx = Math.max(1, Number.isFinite(+idxRaw)?(+idxRaw):1);

  if(WRMODE){
    /* în wrong-mode ignorăm re-randarea din progres, dar scoatem loader-ul la primul snapshot */
    if(FIRST_SNAP){ revealNow(); FIRST_SNAP=false; }
    return;
  }
  ...
  try{ if(!BUF[idx]) await prefetchQuestion(idx); }catch(_){}
  if(FIRST_SNAP){
    CUR=idx;
    ...
    revealNow();
    FIRST_SNAP=false;
    currentShownIndex=CUR;
    return;
  }

  if(idx!==currentShownIndex){
    CUR=idx; currentShownIndex=idx;
    await prefetchAhead(CUR);
    await flipToIndex(CUR,{resetSelections:false});
  }else{
    /* doar ownership schimbat – nu re-randăm întrebarea */
  }
});

/* Fallback: dacă nu se creează doc-ul și user-ul e owner + FIRST_SNAP,
   creez progresul minimal (index 1). Nu stric nimic când există deja. */
setTimeout(async()=>{
  if(FIRST_SNAP && IS_OWNER){
    try{ await progRef.set({currentIndex:1,finished:false,activeSessionId:CLIENT_ID,activeUpdatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true}); }catch(_){}
  }
},1200);

/* ====== Inițializare „final direct” (dacă există deja flag) — lăsăm snapshotul să decidă.
   Dacă intrăm pe pagină și doc-ul e deja finished, onSnapshot va rula showFinal() și va ascunde întrebările. ====== */
