...
(function(){
  ...
  function isTyping(){ var ae=document.activeElement; if(!ae) return false; var t=(ae.tagName||"").toLowerCase(); return (t==="input"||t==="textarea"||ae.isContentEditable===true); }

  // Așteaptă existența window.__ATX (exportat în Partea 1)
  function waitATX(cb, tries){
    tries = tries || 80; // ~3.2s
    (function loop(){
      if (window.__ATX && window.__ATX.state) return cb(window.__ATX);
  ...
  ready(function(){
    waitATX(function(AT){
      if(!AT){ console.warn("[AutoTest P2] __ATX nu este disponibil; patches omise."); return; }

      /* ------- Helpers locale bazate pe REFS curent ------- */
      function REFS(){ try{ return AT.state.REFS; }catch(_){ return null; } }
      function primarySubmit(){ try{ return AT.utils.primarySubmitBtn(); }catch(_){ return null; } }

      // Animă deschiderea/închiderea accordionului cu max-height corect (fără CSS nou).
      function setAccordionOpen(refs, open){
        if(!refs || !refs.acc || !refs.accContent) return;
        // afișăm containerul doar când e nevoie
        refs.acc.style.display = open ? "block" : "none";
        refs.acc.classList.toggle("open", !!open);
        try{
          // max-height animat (folosește deja transition în CSS din Partea 1)
          if(open){
            var h = refs.accContent.scrollHeight || 0;
            refs.accContent.style.maxHeight = (h>0 ? (h+16) : 4000) + "px";
          }else{
            refs.accContent.style.maxHeight = "0px";
          }
        }catch(_){}
        if(refs.accArrow){ try{ refs.accArrow.style.transform = open ? "rotate(180deg)" : ""; }catch(_){ } }
      }

      // Se asigură că butonul Explain chiar deschide/închide accordionul (Partea 1 făcea doar pregătirea).
      document.addEventListener("click", function(e){
        var btn = e.target && e.target.closest && e.target.closest(".explanation-btn,.explanation-card,.explain-btn,.explanation-btn-mobile,.explain-btn-mobile");
        if(!btn) return;
        // Lăsăm handlerul din Partea 1 să ruleze (este pe capture); aici completăm pe bubble.
        setTimeout(function(){
          var r = REFS();
          if(!r) return;
          var open = !(r.acc && r.acc.classList.contains("open")) || (r.accContent && (r.accContent.style.maxHeight==="0px"||!r.accContent.style.maxHeight));
          // Dacă deja este vizibil, respectăm toggle-ul real
          if(r.acc && r.acc.style.display!=="none" && r.acc.classList.contains("open")) open = false;
          setAccordionOpen(r, open);
        }, 0);
      }, false);

      // Suport click direct pe headerul accordionului (dacă utilizatorul apasă acolo)
      document.addEventListener("click", function(e){
        var header = e.target && e.target.closest && e.target.closest(".custom-accordion-header");
        if(!header) return;
        var r = REFS(); if(!r || !r.acc) return;
        // Permitem numai pe wrapperul activ (evităm să alterăm bufferul inactiv)
        var active = AT.utils.activeWrap && AT.utils.activeWrap();
        if(active && !header.closest(active)) return;

        e.preventDefault();
        var opening = !(r.acc.classList.contains("open"));
        setAccordionOpen(r, opening);
      }, true);

      // La flip / re-randare, forțăm ca explicația să fie închisă (în caz că alt script o deschide)
      (function bindFlipCloseExplain(){
        try{
          var _flip = window.flipToIndex;
          if(typeof _flip === "function"){
            window.flipToIndex = async function(idx, opts){
              var out = await _flip(idx, opts);
              try{
                var r = REFS();
                if(r && r.acc){ setAccordionOpen(r, false); }
              }catch(_){}
              return out;
            };
          }
      ...
      // Keyboard QoL: ArrowRight = Next/Finish (dacă suntem în starea ANSWERED), ArrowLeft = nimic (nu avem back), X = închide Explain/AI.
      document.addEventListener("keydown", function(e){
        if(isTyping()) return;
        var key = (e.key||"");
        if(key === "ArrowRight"){
          var btn = primarySubmit();
          if(!btn) return;
          if(btn.classList.contains("is-next") || btn.classList.contains("is-finish") || btn.classList.contains("finish-chapter-btn")){
            e.preventDefault();
            btn.click();
          }
        }else if(key.toUpperCase() === "X"){
          // Închide rapid Explain sau AI pop-up
          var r = REFS();
          var pop = document.querySelector(".ask-ai-pop-up");
          if(pop && pop.style.display!=="none"){
            e.preventDefault();
          ...
          if(r && r.acc && r.acc.classList.contains("open")){
            e.preventDefault();
            setAccordionOpen(r, false);
          }
        }
      }, true);

      // Asigură „AI CHAT” ca text pe butoane după orice flip/randare externă (în caz de intervenții DOM)
      var aiLabelMo = new MutationObserver(function(muts){
        try{
          var r=REFS(); if(!r) return;
          // din Partea 1: applyAIChatText este deja chemat la render; aici doar întărim
          if(r.aiBtn && !/AI CHAT/i.test(r.aiBtn.textContent||"")) r.aiBtn.textContent="AI CHAT";
          if(r.aiBtnMobile && !/AI CHAT/i.test(r.aiBtnMobile.textContent||"")) r.aiBtnMobile.textContent="AI CHAT";
        }catch(_){}
      });
      try{
        aiLabelMo.observe(document.body, {childList:true, subtree:true, attributes:false});
      }catch(_){}

      // În sigurantă, oprește media când se face flip sau se deschide AI
      (function hardStopMediaOnAIOpen(){
        var lastStyle = "";
        setInterval(function(){
          var pop = document.querySelector(".ask-ai-pop-up");
          if(!pop) return;
          var cur = pop.style.display||"";
          if(cur!==lastStyle && cur==="flex"){
            try{ AT.utils.stopActiveMedia && AT.utils.stopActiveMedia(REFS()); }catch(_){}
          }
          lastStyle = cur;
        }, 350);
      })();

      // Cleanup la ieșirea din WRONG-MODE (dacă utilizatorul finalizează direct din ultima întrebare)
      (function ensureWrongClearOnFinish(){
        // interceptăm schimbările de URL spre final screen (fallback) și curățăm wrong-mode persistat
        window.addEventListener("hashchange", function(){
          try{ AT.utils.clearWrongModePersist && AT.utils.clearWrongModePersist(); }catch(_){}
        });
        document.addEventListener("click", function(e){
          var t=e.target && e.target.closest && e.target.closest(".finish-chapter-btn,.is-finish");
          if(!t) return;
          setTimeout(function(){
            try{ AT.utils.clearWrongModePersist && AT.utils.clearWrongModePersist(); }catch(_){}
          }, 0);
      ...
      // Mic guard: dacă loaderul rămâne accidental vizibil >1.5s după init, îl ascundem
      setTimeout(function(){
        try{
          var ld=document.querySelector(".lesson-loader");
          if(ld && getComputedStyle(ld).display!=="none" && !document.body.classList.contains("page-loading")){
            ld.style.setProperty("display","none","important");
          }
        }catch(_){}
      }, 1500);

      // Expunem „Extra” debug (inofensiv)
      if(!window.AutoTestExtra){
        window.AutoTestExtra = Object.freeze({
          openExplain: function(){ var r=REFS(); if(r) setAccordionOpen(r,true); },
          closeExplain:function(){ var r=REFS(); if(r) setAccordionOpen(r,false); },
          next: function(){ var b=primarySubmit(); if(b && (b.classList.contains("is-next")||b.classList.contains("is-finish"))) b.click(); },
          verify: function(){ var b=primarySubmit(); if(b && b.classList.contains("is-verify")) b.click(); }
        });
      }

      console.debug("[AutoTest P2] Patches aplicate ✔");
    });
  });
})();
}); // <-- închide Webflow.push(async function(){ ... })
