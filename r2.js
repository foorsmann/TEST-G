...
const $=(s,p=D)=>p.querySelector(s), a=(s,p=D)=>Array.from(p.querySelectorAll(s));
...
/* ===== Refs & AB double buffer infra ===== */
const QSEC=$(".question-section");
const FINAL=$(".chapter-final-screen");
/* shell & buffers */
const SHELL=$(".question-shell-w")||null;
const WRA=SHELL?.querySelector(".question-wrapper-A")||null;
const WRB=SHELL?.querySelector(".question-wrapper-B")||null;

/* Fallback single wrapper (pentru pagini mai vechi) */
const SINGLE_WRAP=(QSEC&&QSEC.querySelector(".question-wrapper"))||$(".question-wrapper");
...
/* Sanitizare controlată: patch-ul global already set; aici evităm dublu-escape */
const SANITIZE_CONTENT=false;
const sanitizeMaybe=html=>html||"";

/* ===== Date capitol & colecții ===== */
...
/* ===== State ===== */
let BUF={},SEL=[],ANSW=false;
let CUR=1,FIN=false; let WRONG=[],WRMODE=false,WRP=0;
...
function revealNow(){
  ...
  blurActive();
  if(QSEC) QSEC.style.setProperty("display","flex","important");
  const visibleWrap=(SHELL? (SHELL.querySelector(".ab-visible")||SHELL.querySelector(".question-wrapper-A")||SHELL.querySelector(".question-wrapper-B")) : SINGLE_WRAP);
  if(visibleWrap) visibleWrap.classList.add("visible");
  const loader=$(".lesson-loader"); if(loader) loader.style.setProperty("display","none","important");
}

/* ===== Submit state desktop/mobil ===== */
...
/* ===== Refs collector (pentru single sau buffer A/B) ===== */
...
/* Construiește refs pentru un ROOT dat (single sau A/B) */
function collectRefs(ROOT){
  const need=(sel)=>ROOT.querySelector(sel);
  const refs={
    root: ROOT,
    qEl: ROOT.closest(".questions-collection-list-item") || ROOT,
    qWrap: ROOT.classList.contains("question-wrapper") ? ROOT : (need(".question-wrapper")||ROOT),
  ...
  refs.cards = Array.from((refs.optWrap||ROOT).querySelectorAll(".option-card"));
  refs.items = refs.cards.map(c=>c.querySelector(".option-item"));
  refs.staticText = (refs.feedbackWrap||ROOT).querySelector(".correct-answer-static");
  refs.correctAns = (refs.feedbackWrap||ROOT).querySelector(".correct-answer");
...
/* ===== Hydrate & bind pentru un wrapper (single sau A/B) ===== */
function hydrateDomFor(refs){
  if(!refs.qText || !refs.progress || !refs.mainWrap || !refs.optWrap || !refs.cards.length) return false;
    ...
    refs.accHeader.addEventListener("click",async()=>{
      ...
      if(opening){
        if(c){
          ...
          const scroller=acc.closest(".question-wrapper-inner")||getScrollParent(refs.qEl);
  ...
  const onExplainClick=()=>{
    vibrateShort();
    const opening = !isExplanationOpen(refs);
...
/* ===== AB control ===== */
let AB=false;
let buffers={};     // { A:{wrap, refs}, B:{wrap, refs} }
let ACTIVE="A";     // cheia activă
let PREPARED=null;  // "A"/"B" pregătit în fundal
let PREP_IDX=null;  // indexul pregătit în fundal
let refs=null;      // referințele curente (shortcut)

function abShowInstant(keyToShow){
  if(!AB) return;
  const wa=buffers.A.wrap, wb=buffers.B.wrap;
  if(!wa||!wb) return;
  const show = (keyToShow==="A") ? wa : wb;
  const hide = (keyToShow==="A") ? wb : wa;
  hide.classList.add("ab-hidden"); hide.classList.remove("ab-visible");
  show.classList.add("ab-visible"); show.classList.remove("ab-hidden");
}


/* lock pe butoane în timpul tranzițiilor logice (nu vizuale) */
...
/* === Build/upgrade A/B buffers === */
function ensureShellFromSingle(){
  if(!SINGLE_WRAP) return {ok:false};
  // creează shell + 2 layere A/B din SINGLE_WRAP
  const shell=document.createElement("div");
  shell.className="question-shell-w";
  SINGLE_WRAP.parentNode.insertBefore(shell,SINGLE_WRAP);
  const Awrap=SINGLE_WRAP;
  Awrap.classList.add("ab-layer","question-wrapper-A");
  const Bwrap=Awrap.cloneNode(true);
  Bwrap.classList.remove("visible"); // clonă
  Bwrap.classList.add("ab-layer","question-wrapper-B");
  shell.appendChild(Awrap);
  shell.appendChild(Bwrap);
  return {ok:true,shell,Awrap,Bwrap};
}

function initAB(){
  let shell=SHELL, Awrap=WRA, Bwrap=WRB;
  if(!shell || !Awrap || !Bwrap){
    const built=ensureShellFromSingle();
    if(built.ok){
      shell=built.shell; Awrap=built.Awrap; Bwrap=built.Bwrap;
    }
  }
  if(shell && Awrap && Bwrap){
    Awrap.classList.add("ab-layer");
    Bwrap.classList.add("ab-layer");
    // vizibil: doar unul
    Awrap.classList.add("ab-visible"); Awrap.classList.remove("ab-hidden");
    Bwrap.classList.add("ab-hidden");  Bwrap.classList.remove("ab-visible");
    buffers = {
      A: { wrap: Awrap, refs: collectRefs(Awrap) },
      B: { wrap: Bwrap, refs: collectRefs(Bwrap) }
    };
    hydrateDomFor(buffers.A.refs);
    hydrateDomFor(buffers.B.refs);
    refs = buffers.A.refs;
    AB=true;
  }else{
    // fallback single
...
async function showFinal(){
  ...
  try{
    const pool=[];
    const snap2=answersSnap; // deja avem
    snap2.forEach(doc=>{
      const d=doc.data();
      const sel=(d.selected||[]).slice().sort().join(","), cor=(d.correct||[]).slice().sort().join(",");
      if(sel && sel!==cor){
...
async function getNextChapterSlug(){
  ...
  try{
    const s=await db.collection("mediu_invatare_chapters").orderBy("Index").get();
    const arr=[]; s.forEach(d=>arr.push(d.id));
    const i=arr.indexOf(CHAP);
    NEXT_SLUG=(i!==-1&&i<arr.length-1)?arr[i+1]:null;
  }catch(_){ NEXT_SLUG=null; }
  NEXT_READY=true; return NEXT_SLUG;
}

/* === Randare întrebare într-un set de refs === */
async function renderQuestionInto(refs, idx, {resetSelections=true}={}){
  setButtonsTransitionLock(refs,true);
  const q=BUF[idx];
  if(!q){ await showFinal(); setButtonsTransitionLock(refs,false); return; }

  const container=refs.qWrap || refs.root;
  // progress
  if(refs.progress){
    const label = WRMODE ? `${pad2(WRP+1)}/${pad2(WRONG.length)}` : `${pad2(CUR)}/${pad2(TOTAL)}`;
    refs.progress.textContent = label;
  }
  if(QSEC) QSEC.style.setProperty("display","flex","important");
  ...
  // ascunde explicația + AI până după verificare
  ...
  // UI: un singur buton la început (desktop). Mobil rămâne controlat de Webflow.
...
/* === AB helpers === */
function oppositeKey(k){ return k==="A" ? "B" : "A"; }
let CURRENT_IDX=null;

async function renderOnOppositeAndSwap(idx){
  if(!AB){ // fallback single
    await renderQuestionInto(refs, idx, {resetSelections:true});
    CURRENT_IDX=idx;
    return;
  }
  const target = oppositeKey(ACTIVE);
  PREPARED=null; PREP_IDX=null; // vom face render sync acum
  await prefetchQuestion(idx);
  await renderQuestionInto(buffers[target].refs, idx, {resetSelections:true});
  abShowInstant(target);
  ACTIVE=target;
  refs=buffers[ACTIVE].refs;
  CURRENT_IDX=idx;
}

async function prepareIndex(idx){
  if(!AB) return; // în single nu are sens
  if(idx==null) return;
  const target = oppositeKey(ACTIVE);
  PREP_IDX = idx;
  try{
    ...
    PREPARED=null; PREP_IDX=null;
  }
}

/* === Flux inițial: progres & wrong-mode persist === */
...
/* === Hydrate & primă randare === */
if(!refs || !refs.qText || !refs.progress){ alert("Structura nu e completă. Verifică clasele HTML."); document.body.classList.remove("page-loading"); }
else{
  if(persistedWM && persistedWM.active && (persistedWM.list||[]).length){
    ...
    await renderOnOppositeAndSwap(WRONG[WRP]);
    ...
    await renderOnOppositeAndSwap(CUR);
    await prefetchAhead(CUR);
    revealNow();
  }
}

/* === Get active index helper === */
const getActiveIndex = ()=> WRMODE ? WRONG[WRP] : CUR;

/* === Submit/NEXT/FINISH handler (desktop + mobil) === */
(QSEC||document).addEventListener("click", async (e)=>{
  ...
  if(isNext && ANSW){
    ...
    if(WRMODE){
      ...
      if(AB && PREPARED && PREP_IDX===nextIdx){
        abShowInstant(PREPARED);
        ACTIVE=PREPARED; refs=buffers[ACTIVE].refs; CURRENT_IDX=nextIdx; PREPARED=null; PREP_IDX=null;
      }else{
        await renderOnOppositeAndSwap(nextIdx);
      }
      await prefetchAhead(nextIdx);
      pruneBUF();
      SEL=[]; ANSW=false;
      setButtonsTransitionLock(refs,false);
      return;
  ...
  if(isVerify){
    ...
    // pregătim din timp următoarea întrebare în bufferul opus
    const nextIdx = WRMODE ? WRONG[WRP+1] : (CUR<TOTAL?CUR+1:null);
    if(nextIdx!=null) prepareIndex(nextIdx);
  }
});

/* === Wrong list builder & starter === */
async function buildWrongListNow(){
  const pool=[];
  const snap=await ansCollection.where("chapterSlug","==",CHAP).get();
  snap.forEach(doc=>{
    const d=doc.data();
    const sel=(d.selected||[]).slice().sort().join(","), cor=(d.correct||[]).slice().sort().join(",");
    if(sel && sel!==cor){
...
async function startWrongModeSafe(){
  ...
  try{
    ...
    SEL=[]; ANSW=false;
    if(AB && PREPARED && PREP_IDX===list[0]){
      abShowInstant(PREPARED);
      ACTIVE=PREPARED; refs=buffers[ACTIVE].refs; CURRENT_IDX=list[0]; PREPARED=null; PREP_IDX=null;
    }else{
      await renderOnOppositeAndSwap(list[0]);
    }
    await prefetchAhead(list[0]||0);
...
/* === Delegări ecran final & acțiuni globale === */
document.addEventListener("click",async e=>{
  ...
  if(e.target.closest(".confirm-reset-chapter")){
    vibrateShort(); stopActiveMedia(refs); const aw=document.querySelector(".alert-wrapper");
    try{
