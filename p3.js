...
/* ---------- Loader inițial (lăsat ca la tine) ---------- */
...
/* Opțional: lock single-active dacă e injectat în pagină */
...
/* ---------- Sanitizare conținut dinamic (lăsăm patch-ul inițial să lucreze) ---------- */
const SANITIZE_CONTENT=false;
const sanitizeMaybe=html=>html||""; // A11Y patch din Webflow va face hard-sanitize pe zonele sensibile
...
/* persist localStorage pentru wrong-mode */
const LS_WRONG_KEY=`wm:${user.uid}:${CHAP}`;

/* ---------- Persist helpers pentru wrong-mode ---------- */
...
/* ---------- Double Buffer A/B: folosim strict clasa .is-active ---------- */
const SHELL = $(".question-shell-w") || QSEC || D;
const WA = $(".question-wrapper-a", SHELL);
const WB = $(".question-wrapper-b", SHELL);
if(!SHELL || !WA || !WB){
  alert("Structura A/B lipsește: adaugă .question-wrapper-a și .question-wrapper-b în .question-shell-w");
  return;
}
/* La load: ne asigurăm că e activă doar una */
(function(){
...
function getScrollParent(el){
  ...
  let p=el?.parentElement; while(p){ const s=getComputedStyle(p),oy=s.overflowY; if(oy==="auto"||oy==="scroll"||oy==="overlay") return p; p=p.parentElement; }
...
/* ---------- Refs colectate per wrapper ---------- */
function collectRefs(wrapper){
  const root=wrapper, need=(sel)=>root.querySelector(sel);
  const refs={
    root,
    ...
    cards:Array.from(root.querySelectorAll(".option-card")),
    items:[],
    ...
    /* mobil */
    buttonsWrapMobile:need(".submit-next-buttons-mobile")||$(".submit-next-buttons-mobile"),
  ...
  if(refs.buttonsWrapMobile){
    refs.submitBtnMobile   =refs.buttonsWrapMobile.querySelector(".submit-btn-mobile")||refs.buttonsWrapMobile.querySelector(".submit-btn");
    refs.explainBtnMobile  =refs.buttonsWrapMobile.querySelector(".explanation-btn-mobile")||refs.buttonsWrapMobile.querySelector(".explanation-btn");
    refs.aiBtnMobile       =refs.buttonsWrapMobile.querySelector(".ask-ai-btn-mobile")||refs.buttonsWrapMobile.querySelector(".ask-ai-btn");
  }
  return refs;
}
...
/* ---------- Locks pe butoane + single-btn state ---------- */
...
async function renderQuestionInWrapper(refs,idx,{resetSelections=false}={}){
  ...
  refs.qText && (refs.qText.innerHTML="");
  refs.qText && refs.qText.insertAdjacentHTML("afterbegin",sanitizeMaybe(q.Question||""));
  ...
  /* Reset UI de explicații până după verificare */
  ...
  /* Desktop: un singur buton la început; mobil – layout strict Webflow */
  setSingleBtnState(refs,true);
  setSubmitStateBoth(refs,"verify");

  /* Imagine (prefetch + decode asigurate înainte) */
  ...
  /* Restore răspuns (cu filtrare la wrong-mode pe sesiune curentă) */
  try{
    ...
    if(snap.exists){
      ...
      if(allowRestore && selSaved.length>0){
        ...
        const ord=["A","B","C"].filter(x=>corSaved.includes(x)); refs.correctAns&&(refs.correctAns.textContent=ord.join(", "));
        ...
        forceAIChatText(refs.aiBtn); forceAIChatText(refs.aiBtnMobile);
...
function bindAI(refs){
  const onAiClick=()=>{ vibrateShort(); const v=refs.videoWrap?.querySelector("video"); if(v && !v.paused){ try{ v.pause(); if(v.plyr) v.plyr.pause(); }catch(_){ } } ensureAIPop().style.display="flex"; D.body.style.overflow="hidden"; };
...
/* ---------- Hydrate ambele buffere ---------- */
let REFS_A=collectRefs(WA);
let REFS_B=collectRefs(WB);
[REFS_A,REFS_B].forEach(r=>{ bindSelection(r); bindAccordion(r); bindExplain(r); bindAI(r); });

/* ---------- Active/Buffer helpers + flip ---------- */
const ACTIVE_CLS="is-active";
function activeWrap(){return WA.classList.contains(ACTIVE_CLS)?WA:WB;}
function bufferWrap(){return WA.classList.contains(ACTIVE_CLS)?WB:WA;}
function refsOf(el){return el===WA?REFS_A:REFS_B;}
function getActiveRefs(){ return refsOf(activeWrap()); }
...
async function flipToIndex(idx,{resetSelections=false}={}){
  ...
  act.classList.remove(ACTIVE_CLS);
  buf.classList.add(ACTIVE_CLS);
  /* Instant: nu atingem opacity/transition aici; Webflow controlează vizibilitatea */
}
...
/* ---------- INIT (cu reluare wrong-mode dacă există) ---------- */
...
if(persistedWM && persistedWM.active && (persistedWM.list||[]).length){
  ...
  await renderQuestionInWrapper(getActiveRefs(),WRONG[WRP],{resetSelections:true});
  revealNow();
}else if(FIN){
  ...
  await renderQuestionInWrapper(getActiveRefs(),CUR,{resetSelections:false});
  revealNow();
}
...
/* Media stop în wrapperul activ */
function stopActiveMediaIn(refs){
...
/* Index activ (normal / wrong-mode) */
function getActiveIndex(){ return WRMODE ? WRONG[WRP] : CUR; }

/* Prune buffer cache (light) */
function pruneBUF(){
  const keep=new Set();
  if(!WRMODE){
    [CUR-1, CUR, CUR+1, CUR+2].forEach(i=>{ if(i>=1 && i<=TOTAL) keep.add(i); });
  }else{
    [WRONG[WRP-1], WRONG[WRP], WRONG[WRP+1], WRONG[WRP+2]]
...
/* Final screen (refactor din P1 ca funcție reutilizabilă) */
...
/* Next chapter helper (ordinea după Index) */
...
/* WRONG list (din răspunsuri salvate) */
async function buildWrongListNow(){
  ...
  snap.forEach(doc=>{
    ...
    if(sel && sel!==cor){
      ...
      if(Number.isFinite(idx)) pool.push(idx);
    }
  });
  return Array.from(new Set(pool)).sort((a,b)=>a-b);
}

/* ---------- WRONG start safe (prefetch + persist + flip) ---------- */
async function startWrongModeSafe(){
  ...
  const actRefs=getActiveRefs();
  const container=actRefs?.qWrap||actRefs?.root||document.body;
  ...
  try{
    ...
    try{ await ensureImageReadyFromQ(BUF[list[0]]); }catch(_){}
...
/* ---------- Submit state helpers pe wrapper activ ---------- */
function setSubmitStateBothOnActive(state){
  const refs=getActiveRefs();
  setSubmitStateBoth(refs,state);
}
function setSingleBtnStateOnActive(on){
  const refs=getActiveRefs();
  setSingleBtnState(refs,on);
}

/* ---------- Delegare CLICK pentru SUBMIT (desktop + mobil) ---------- */
document.addEventListener("click",async(e)=>{
  ...
  if(isNext && ANSW){
    ...
    if(WRMODE){
      ...
      await persistWrongMode({active:true,list:WRONG,pos:WRP});
      await prefetchAhead(WRONG[WRP]||0); pruneBUF();
      SEL=[]; ANSW=false;
      await flipToIndex(WRONG[WRP],{resetSelections:true});
      ...
      if(CUR<TOTAL){
        ...
        await prefetchAhead(CUR); pruneBUF();
        SEL=[]; ANSW=false;
  ...
  if(isVerify){
    if(SEL.length===0){
      ...
      playOne(alertSound);
      setSingleBtnStateOnActive(true);
      return;
    }
    const q=BUF[idx]; if(!q) return;
    ...
    const selected=SEL.slice().sort().join(",");
    const correct =cor.slice().sort().join(",");
    ...
    /* Vizibilizare explicație + AI (desktop + mobil) */
    refs.feedbackWrap?.style.setProperty("display","flex","important");
    if(refs.explainBtn) refs.explainBtn.style.setProperty("display","flex","important");
    if(refs.explainBtnMobile) refs.explainBtnMobile.style.setProperty("display","flex","important");
    if(refs.aiBtn) refs.aiBtn.style.setProperty("display","flex","important");
    ...
    setSingleBtnStateOnActive(false);
    const lastWrong=WRMODE&&(WRP===WRONG.length-1),lastNormal=!WRMODE&&(CUR===TOTAL);
    setSubmitStateBothOnActive((lastWrong||lastNormal)?"finish":"next");
...
document.addEventListener("click",async e=>{
  /* Refă doar greșitele */
  if(e.target.closest(".reset-wrong-questions-only")){
    await startWrongModeSafe(); 
    return;
  }
  /* Următorul capitol */
  if(e.target.closest(".next-chapter-btn")){
    vibrateShort(); stopActiveMediaIn(getActiveRefs()); await clearWrongModePersist();
    const next=await getNextChapterSlug(); if(next) location.replace(`${location.pathname}?chapter=${encodeURIComponent(next)}`);
    return;
  }
  /* Reset capitol – confirm dialog clasic (wrapper separat de toast) */
  if(e.target.closest(".reset-chapter-btn")){ 
    vibrateShort();
    const aw=$(".alert-wrapper"); /* acesta e overlay-ul de confirmare */
    if(aw) aw.style.display="flex";
    return; 
  }
  /* Confirm reset capitol */
  if(e.target.closest(".confirm-reset-chapter")){
    vibrateShort(); 
    stopActiveMediaIn(getActiveRefs()); 
...
/* ---------- Enter = submit primar, Escape = închidere overlay-uri ---------- */
...
/* ---------- Idempotent: segmente pentru bara finală, dacă lipsesc ---------- */
