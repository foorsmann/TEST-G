(function(){
  // ===== Guard anti-dublare (și evită top-level let/const) =====
  if (window.__MEDIU_INIT_DONE__) return;
  window.__MEDIU_INIT_DONE__ = true;
  ...
  // NO-IMAGE state — aplicat pe wrapperul activ, nu global
  ...
  // ==== RENDER întrebare în wrapper (cu skipRestore pt. pre-render) ====
  async function renderQuestionInWrapper(refs,idx,{resetSelections=false,skipRestore=false}={}){
    ...
    // Restore răspuns doar în randarea „reală”, nu în pre-render
    if(!WRMODE && !skipRestore){
      try{
  ...
  if(FIN){
    ...
    // showFinal se definește mai jos și va fi chemat când e nevoie
    D.body.classList.remove("page-loading");
  }else{
  ...
  // ===== Pre-render „următoarea” întrebare în buffer =====
  function schedulePreRenderNext(){
    const nextIdx = WRMODE ? (WRONG[WRP+1]) : (CUR<TOTAL?CUR+1:null);
    if(!nextIdx) return;
    const buf = getBufferWrap();
    const bufRefs = collectRefs(buf);
    buf.dataset.prerenderIdx = String(nextIdx);
    // Pre-randare fără restore din Firestore (evit „extra reads”)
    renderQuestionInWrapper(bufRefs, nextIdx, { resetSelections:true, skipRestore:true });
    // Cache în fundal încă una (imagine/video)
    prefetchAhead(WRMODE?WRONG[WRP]:CUR);
  }

  // ===== Flip cu buffer deja pregătit =====
  async function flipToIndex(idx,{resetSelections=false}={}){
    const act = getActiveWrap();
    const buf = getBufferWrap();
    const actRefs = collectRefs(act);
    const bufRefs = collectRefs(buf);

    // Oprește media în activ (non-blocking)
    stopActiveMedia(actRefs);

    // Dacă bufferul e deja pre-rendat pt idx -> nu mai așteptăm
    const preReady = (buf.dataset.prerenderIdx===String(idx));
    if(!preReady){
      await renderQuestionInWrapper(bufRefs, idx, { resetSelections:true, skipRestore:true });
    }

    // Flip instant
    act.classList.remove("is-active","visible");
    buf.classList.add("is-active","visible");
    delete buf.dataset.prerenderIdx;

    // Actualizează REFS curente
    REFS = (buf===WA) ? (REFS_A=collectRefs(WA)) : (REFS_B=collectRefs(WB));

    // Pre-randare pentru următoarea
    schedulePreRenderNext();
  }


  // ---- finalize active wrapper după flip: doar restore stare, fără re-render complet
  async function restoreSelectionFromFirestore(refs, idx){
    if(WRMODE) return; // nu restaurăm în wrong-mode
    try{
      const snap=await ansCollection.doc(`${CHAP}_${idx}`).get();
      if(!snap.exists){ SEL=[]; ANSW=false; refs.buttonsWrap?.classList.add("single-btn-state"); setSubmitState(refs.submitBtn,"verify"); return; }
      const d=snap.data();
      ...
      if(selSaved.length>0){
        ...
        ANSW=true;
        applySelectedVisualsFromSEL(refs.root);
        clearLetterStates(refs.root); applyLetterStatesByCorrect(refs.root,corSaved);
        clearWrongCards(refs.root);   applyWrongCardsByCorrect(refs.root,corSaved);
        const same=SEL.slice().sort().join(",")===corSaved.slice().sort().join(",");
        ...
        SEL=[]; ANSW=false; refs.buttonsWrap?.classList.add("single-btn-state"); setSubmitState(refs.submitBtn,"verify");
      }
    }catch(e){ console.warn("Restore (fast) failed:",e); }
  }

  // === FINAL SCREEN ===
  async function showFinal(){
    try{
      const chapterTextFinalEl=$(".chapter-text-final");
      if(chapterTextFinalEl){
        const chDoc=await firebase.firestore().collection("mediu_invatare_chapters").doc(CHAP).get();
        if(chDoc.exists) chapterTextFinalEl.textContent=chDoc.data().Name;
      }
    }catch(_){}
    ...
    function ensureFinalSegmentsLocal(){
      if(!barOuterFinal) return;
      if(!barGreenFinal){barGreenFinal=D.createElement("span");barGreenFinal.className="progress-green";barOuterFinal.appendChild(barGreenFinal);}
      if(!barRedFinal){barRedFinal=D.createElement("span");barRedFinal.className="progress-red";barOuterFinal.appendChild(barRedFinal);}
      if(!barGapFinal){barGapFinal=D.createElement("span");barGapFinal.className="progress-gap";barOuterFinal.appendChild(barGapFinal);}
    }
    function applyFinalBarBaseStylesLocal(){
      if(!barOuterFinal) return; ensureFinalSegmentsLocal();
      Object.assign(barOuterFinal.style,{overflow:"hidden",position:"relative",display:"flex",borderRadius:`${RADIUS_PX}px`});
      if(barGreenFinal) Object.assign(barGreenFinal.style,{position:"absolute",left:"0",top:"0",bottom:"0",borderRadius:`${RADIUS_PX}px`,transition:"left .25s ease,width .25s ease"});
      if(barRedFinal)   Object.assign(barRedFinal.style,{position:"absolute",top:"0",bottom:"0",borderRadius:`${RADIUS_PX}px`,transition:"left .25s ease,width .25s ease"});
      if(barGapFinal)   Object.assign(barGapFinal.style,{position:"absolute",top:"0",bottom:"0",width:`${GAP_PX}px`,backgroundColor:GAP_COLOR,display:"none",pointerEvents:"none",borderRadius:`${RADIUS_PX}px`});
    ...
    let finalPercentEl=null;
    function ensureFinalPercentElLocal(){
      if(!progressCounterFinal) return null;
      finalPercentEl=progressCounterFinal.querySelector(".exam-final-percent");
      if(!finalPercentEl){
        finalPercentEl=D.createElement("span"); finalPercentEl.className="exam-final-percent";
    ...
    const setFinalPercentLocal=v=>{ensureFinalPercentElLocal();if(finalPercentEl) finalPercentEl.textContent=String(v);};
    function animateFinalProgressLocal(cPct,wPct,d=ANIM_DUR){
      if(!barOuterFinal) return; applyFinalBarBaseStylesLocal();
      const answeredPct=Math.max(0,Math.min(100,(+cPct||0)+(+wPct||0))),hasBoth=(+cPct>0)&&(+wPct>0);
      if(answeredPct>0&&+cPct>0){ if(hasBoth){barGreenFinal.style.width=`calc(${(cPct/answeredPct)*100}% - ${GAP_PX/2}px)`;barGreenFinal.style.left="0";} else{barGreenFinal.style.width="100%";barGreenFinal.style.left="0";} }
      else{barGreenFinal.style.width="0%";barGreenFinal.style.left="0";}
      if(answeredPct>0&&+wPct>0){
      ...
      if(barGapFinal){ if(hasBoth){ const leftPct=(cPct/answeredPct)*100; barGapFinal.style.display="block"; barGapFinal.style.left=`calc(${leftPct}% - ${GAP_PX/2}px)`; } else barGapFinal.style.display="none"; }
      barOuterFinal.style.transition=`width ${d}ms cubic-bezier(.68,-0.55,.27,1.55)`; barOuterFinal.style.width="0%";
      if(progressCounterFinal){ setFinalPercentLocal("0%"); const start=performance.now(); const step=ts=>{const p=Math.min((ts-start)/d,1),val=Math.round(p*(+cPct||0)); setFinalPercentLocal(val+"%"); if(p<1) requestAnimationFrame(step);}; requestAnimationFrame(step); }
      requestAnimationFrame(()=>{ setTimeout(()=>{ barOuterFinal.style.width=`${answeredPct}%`; },40); });
    }
  ...
  async function startWrongModeSafe(){
    ...
    const act=getActiveWrap(); const actRefs=collectRefs(act);

    // „îngheață” UI pe înălțimea existentă
    ...
    try{
      ...
      // Pre-render pe buffer, apoi flip
      await flipToIndex(WRONG[WRP],{resetSelections:true});
      // Restaurează starea dacă există
      await restoreSelectionFromFirestore(REFS, WRONG[WRP]);
  ...
  QSEC?.addEventListener("click",async(e)=>{
    ...
    const act=getActiveWrap(), refs=collectRefs(act);
    ...
    if(isNext && ANSW){
      if(WRMODE){
        ...
        if(CUR<TOTAL){
          ...
          await restoreSelectionFromFirestore(REFS, CUR);
          return;
        }else{
    ...
    if(isVerify){
      ...
      // Pre-render next imediat după verificare
      schedulePreRenderNext();
    }
  });
  ...
  // Enter pentru submit / Escape pentru închideri
  ...
  // După prima randare: pre-render next
  if(!FIN){ schedulePreRenderNext(); }

  // Dacă era deja finalizat înainte să se definească showFinal()
  if(FIN){ await showFinal(); }
