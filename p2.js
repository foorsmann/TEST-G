...
/* ---------- Double Buffer A/B: folosim strict clasa .is-active (Webflow controlează vizibilitatea) ---------- */
...
/* La load: Webflow are deja .is-active pe A. Ne asigurăm că nu sunt ambele active. */
(function(){
...
/* ---------- Refs colectate per wrapper (desktop + mobil) ---------- */
function collectRefs(wrapper){
  ...
  const refs={
    ...
    /* mobil (lăsăm layout 100% Webflow) */
...
/* ---------- Render întrebare în wrapper (NU schimbăm clase CSS din Webflow, doar conținutul) ---------- */
...
async function flipToIndex(idx,{resetSelections=false}={}){
  ...
  /* După flip, nu atingem z-index/opacity prin JS — Webflow le controlează prin clase. */
}
...
function revealNow(){
  ...
  blurActive();
  if(QSEC) QSEC.style.setProperty("display","flex","important");
  /* Nu mai adăugăm .visible pe wrappere; is-active + stilurile din Webflow sunt suficiente */
...
/* Delegare CLICK pentru SUBMIT (desktop + mobil, ambele buffere) */
...
document.addEventListener("click",async e=>{
  ...
  if(e.target.closest(".reset-chapter-btn")){ 
    vibrateShort(); const aw=$(".alert-wrapper"); if(aw) aw.style.display="flex"; return; 
  }
  if(e.target.closest(".confirm-reset-chapter")){
    vibrateShort(); stopActiveMediaIn(getActiveRefs()); const aw=$(".alert-wrapper");
    try{
...
/* Enter = submit primar (mobil dacă e vizibil), Escape = închidere overlay-uri */
...
/* Idempotent final: segmente bară finală dacă lipsesc */
