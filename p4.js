...
/* Opțional: lock single-active dacă e injectat în pagină (îl lași în Webflow dacă vrei) */
...
function toast(txt,dur=1000){ if(!AWrap) return;
...
function hideAlertBox(){ if(!AWrap) return;
  if(alertBoxTimeout){clearTimeout(alertBoxTimeout);alertBoxTimeout=null;}
...
/* ---------- Sanitizare dinamică ----------
   Lăsăm patch-ul existent din Webflow să intercepteze insertAdjacentHTML.
   Aici doar bypass (dacă patch-ul lipsește, HTML-ul vine din DB-ul tău).
*/
...
/* wrong-list precomputation (opțional) */
let WRONG_PREPARED=[],WRONG_PREPARED_READY=false;
/* guard pentru pornire wrong-mode (UN SINGUR DECLARAT) */
...
/* La load: A are .is-active; ne asigurăm că nu sunt ambele active. */
(function(){
...
/* ---------- Locks butoane + single-btn state ---------- */
...
async function renderQuestionInWrapper(refs,idx,{resetSelections=false}={}){
  ...
  /* Restore răspuns (filtrat pentru wrong-mode doar în sesiunea curentă) */
  try{
...
async function flipToIndex(idx,{resetSelections=false}={}){
  ...
  /* Instant: CSS-ul tău controlează vizibilitatea .is-active */
}
...
/* ---------- INIT (reluare wrong-mode dacă există) ---------- */
...
/* Idempotent: segmente bară finală dacă lipsesc (o dată la init) */
