...
if (window.installSingleActiveLock) {
  window.installSingleActiveLock({
    lockKey: "chapter:" + CHAP,                 // unic per capitol
    redirectUrl: "https://webarcs-ultra-awesome-site.webflow.io/mediu-invatare",
    userId: user.uid                            // folosim direct uid-ul curent
  });
}
...
/* HYDRATE + Soluția A pentru acordion (deschidere instant + un scroll) */
function hydrateExistingDom(){
  ...
  /* Soluția B (robustă) – controlăm săgeata exclusiv prin clasa .open (fără stiluri inline) */
  ...
  if(refs.explainBtn && !refs.explainBtn.dataset.boundExp){
    refs.explainBtn.addEventListener("click",()=>{
      ...
      if(opening){
        ...
        // săgeata rămâne jos implicit (fără inline transform)
        ...
        // curățăm explicit orice transform inline rămas din versiuni vechi
        if(refs.accArrow){ try{ refs.accArrow.style.removeProperty("transform"); }catch(_){} }
...
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  // fix: asigurăm resetul oricărui transform inline pe săgeată, pentru a lăsa CSS-ul să decidă
  if(refs.accArrow){ try{ refs.accArrow.style.removeProperty("transform"); }catch(_){} }
...
/***** WRONG MODE – varianta robustă anti-flash *****/
...
async function startWrongModeSafe(){
  ...
  /* 1) Înghețăm UI-ul de întrebare (nu arătăm „ultima întrebare”) */
  if(container){
  ...
  try{
    /* 2) Alegem lista „wrong”: preferăm cea pregătită; dacă nu e gata, o calculăm acum */
    ...
    /* 3) Prefetch puternic (inclusiv imaginea primei întrebări) înainte de swap */
    ...
    /* 4) Activăm wrong-mode și pregătim bufferul + indexul  */
    WRMODE=true; WRP=0; WRONG=list.slice(0);
    /* menținem BUF existent (deja avem q1 + restul încărcate), doar curățăm excesul */
    pruneBUF();

    /* 5) Populate atomic, apoi decongelăm într-un singur frame */
    SEL=[]; ANSW=false;
