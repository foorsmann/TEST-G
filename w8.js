...
/* ===== WRONG MODE PERSIST: Firestore + localStorage fallback ===== */
const LS_WRONG_KEY=`wm:${user.uid}:${CHAP}`;
async function persistWrongMode({active=WRMODE,list=WRONG,pos=WRP}={}){
  try{
    await progRef.set({wrongMode:{active, list:Array.isArray(list)?list.slice(0,500):[], pos:Math.max(0,Math.min((list?.length||1)-1, +pos||0)), updatedAt:firebase.firestore.FieldValue.serverTimestamp()}},{merge:true});
  }catch(_){}
  try{ localStorage.setItem(LS_WRONG_KEY, JSON.stringify({a:!!active,l:(list||[]),p:+pos||0,t:Date.now()})); }catch(_){}
}
function loadWrongModeLocal(){
  try{ const raw=localStorage.getItem(LS_WRONG_KEY); if(!raw) return null; const v=JSON.parse(raw); if(!v||typeof v!=="object") return null; return {active:!!v.a,list:Array.isArray(v.l)?v.l:[],pos:Math.max(0,+v.p||0)}; }catch(_){ return null; }
}
async function clearWrongModePersist(){
  try{ await progRef.set({wrongMode:{active:false,list:[],pos:0,updatedAt:firebase.firestore.FieldValue.serverTimestamp()}},{merge:true}); }catch(_){}
...
/* HYDRATE + acordion robust (fără stil inline pe săgeată) */
...
/* ===== INIT cu suport de reluare Wrong-Mode ===== */
...
/* Dacă în progres există wrongMode activ, reluăm exact acolo (fără final-screen, fără restore de selecții) */
let INIT_RAN_WRONG=false;
try{
  const pm=progSnap.exists?progSnap.data().wrongMode:null;
  let active=!!pm?.active, list=Array.isArray(pm?.list)?pm.list:[], pos=+pm?.pos||0;
  if(!active){ const loc=loadWrongModeLocal(); if(loc&&loc.active){ active=true; list=loc.list; pos=loc.pos; } }
  if(active && list.length){
    WRMODE=true; WRONG=Array.from(new Set(list.map(v=>+v).filter(n=>Number.isFinite(n)&&n>=1))).sort((a,b)=>a-b);
    WRP=Math.max(0,Math.min(WRONG.length-1,pos));
    FIN=false;
    await prefetchMany([WRONG[WRP]],{[WRONG[WRP]]:"auto"});
...
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isFinish && ANSW){
    ...
    if(WRMODE){ WRMODE=false; await clearWrongModePersist(); }
  ...
  if(isVerify){
    ...
    Promise.all([
      ...
      if(WRMODE){ persistWrongMode({active:true,list:WRONG,pos:WRP}); }
    });
  }
});


/***** WRONG MODE – varianta robustă + persistență *****/
...
async function startWrongModeSafe(){
  ...
  try{
    // 2) Listă wrong: preferăm cea pregătită, altfel construim
    ...
    // 4) Activăm wrong-mode
    WRMODE=true; WRP=0; WRONG=list.slice(0);
    pruneBUF();

    // 5) Populate atomic, persist, decongelăm
    SEL=[]; ANSW=false;
    await updateInPlace(WRONG[WRP],{resetSelections:true});
    await prefetchAhead(WRONG[WRP]||0);
    await persistWrongMode({active:true,list:WRONG,pos:WRP});
...
/***** Final screen — delegări globale (folosește varianta safe) *****/
D.addEventListener("click",async e=>{
  if(e.target.closest(".reset-wrong-questions-only")){ await startWrongModeSafe(); return; }
  if(e.target.closest(".next-chapter-btn")){ vibrateShort(); stopActiveMedia(); const next=await getNextChapterSlug(); if(next) location.replace(`${location.pathname}?chapter=${encodeURIComponent(next)}`); return; }
