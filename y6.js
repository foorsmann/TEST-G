...
/* NOTĂ: login inițial neschimbat pentru stabilitate */
...
/* toast cu ARIA live update */
function toast(txt,dur=1000){
  if(!AWrap) return;
  const t=findAlertTextEl(AWrap); if(t) t.textContent=txt;
  /* ARIA live pentru screen-readers */
  const live=document.getElementById("aria-live");
  if(live){ live.textContent=""; setTimeout(()=>{ live.textContent=txt; },0); }
...
/* Capitole & TOTAL cu count() + fallback */
...
/* >>>>>> NOU: count() aggregate cu fallback .get().size */
async function getTotalCount(){
  try{
    const agg=await db.collection("mediu_invatare_pages")
      ...
      .count()
      .get({source:"server"});
    const n=agg && typeof agg.data==="function" ? agg.data().count : (agg?.data?.count ?? 0);
...
/* >>>>>> NOU: prefetchMany cu batch-uri de 10 + split number/string + fallback sigur */
async function prefetchMany(indices, priorityMap = {}){
  const raw=(indices||[])
    .map(v=>Number.isFinite(+v)?+v:v)
    .filter(v=>v && (!TOTAL || +v<=TOTAL) && !BUF[v]);
  ...
  const runChunk=async (vals, asString=false)=>{
    for(const pack of chunk(vals,10)){
      try{
        ...
        qs.forEach(doc=>{
          ...
          if(d.Image){
            ...
            try{
              ...
              im.src=d.Image;
              const ready=im.decode?im.decode():new Promise(r=>{im.onload=im.onerror=r;});
        ...
        /* fallback sigur per item ca să nu pierzi nimic */
...
function hydrateExistingDom(){
  ...
  /* toggling explicații – sincron desktop + mobil */
...
/* === INIT cu reluare wrong-mode (dacă exista) — continuare flux urmează în Partea 2 === */

/* >>>>>> NOU: Shortcuturi tastatură A/B/C când nu e verificat */
document.addEventListener("keydown", e=>{
  if(ANSW) return;
  const k=(e.key||"").toUpperCase();
  if(k==="A"||k==="B"||k==="C"){
    const map={A:0,B:1,C:2}, idx=map[k];
    const card=refs?.cards && refs.cards[idx];
    if(card && card.style.display!=="none"){ e.preventDefault(); card.click(); }
  }
});
...
/* === Utilitar: litere corecte din DB === */
...
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    if(SEL.length===0){
      ...
      try{ alertSound.pause(); alertSound.currentTime=0; alertSound.play(); }catch(_){}
      setSingleBtnState(true);
      return;
    }
    ...
    const lottieEl=refs.qEl.querySelector(".lottie-animation-correct");
    if(isFullyCorrect){
      try{ correctSound.pause(); correctSound.currentTime=0; correctSound.play(); }catch(_){}
      if(lottieEl){
      ...
      try{ wrongSound.pause(); wrongSound.currentTime=0; wrongSound.play(); }catch(_){}
      if(lottieEl){ lottieEl.style.setProperty("display","none","important"); lottieEl.innerHTML=""; }
    }
...
/* === QoL: Enter=submit primar, Esc=închide overlay-uri === */
