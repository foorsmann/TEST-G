...
/* PREP wrong-list pentru start instant */
let WRONG_PREPARED=[],WRONG_PREPARED_READY=false,START_WRONG_IN_PROGRESS=false;

/* === Persistență WRONG-ONLY === */
const persistWrong = (st)=>progRef.set(st,{merge:true}).catch(()=>{});
function persistWrongNow(){
  if(!WRMODE) return;
  persistWrong({wrongModeActive:true,wrongList:WRONG.slice(),wrongPointer:WRP,wrongFinished:false,finished:false});
}
window.addEventListener("beforeunload",()=>{ try{persistWrongNow();}catch(_){}});
...
async function prefetchMany(indices,priorityMap={}){
  ...
  try{
    const qs=await db.collection("mediu_invatare_pages")
      ...
      .where("Index","in",list.slice(0,10)).get();
    qs.forEach(doc=>{
    ...
    await Promise.all(list.map(v=>prefetchQuestion(v)));
  }
}
...
/* HYDRATE + acordion fără transform inline (doar clasa .open conduce săgeata) */
function hydrateExistingDom(){
  ...
  // siguranță: eliminăm orice transform inline rezidual pe săgeată
...
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  refs.buttonsWrap.classList.add("single-btn-state"); setSubmitState(refs.submitBtn,"verify");
  ...
  /* >>> persist punct curent dacă suntem în WRONG MODE */
  if(WRMODE){ persistWrongNow(); }
...
/* ==== INIT cu restaurare WRONG-ONLY dacă e activ ==== */
const progSnap=await progRef.get();
let WRONG_ACTIVE=false, WRONG_DATA=[], WRONG_PTR=0, WRONG_DONE=false;
if(progSnap.exists){
  const pd=progSnap.data();
  if(pd.currentIndex) CUR=pd.currentIndex;
  if(pd.finished) FIN=true;
  WRONG_ACTIVE=!!pd.wrongModeActive;
  WRONG_DATA=Array.isArray(pd.wrongList)?pd.wrongList.map(v=>parseInt(v,10)).filter(n=>Number.isFinite(n)):[];
  WRONG_PTR=Number.isFinite(pd.wrongPointer)?pd.wrongPointer:0;
  WRONG_DONE=!!pd.wrongFinished;
}
if(!hydrateExistingDom()){ alert("Structura nu e completă. Verifică clasele HTML."); D.body.classList.remove("page-loading"); return; }

/* dacă există sesiune wrong-only activă, o reluăm PRIORITAR */
if(WRONG_ACTIVE && !WRONG_DONE && WRONG_DATA.length){
  WRMODE=true; WRONG=WRONG_DATA.slice(); WRP=Math.max(0,Math.min(WRONG_PTR,WRONG.length-1));
  ...
  if(FIN){ await showFinal(); D.body.classList.remove("page-loading"); }
  else{ await prefetchQuestion(CUR); await prefetchAhead(CUR); await updateInPlace(CUR,{resetSelections:false}); revealNow(); }
}
...
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isFinish && ANSW){
    ...
    WRMODE=false; await persistWrong({wrongModeActive:false,wrongList:[],wrongPointer:0,wrongFinished:true}); await showFinal(); return;
  }
  ...
  if(isNext && ANSW){
    ...
    if(WRMODE){
      ...
      if(WRP>=WRONG.length){
        WRMODE=false;
        try{
          await persistWrong({wrongModeActive:false,wrongList:[],wrongPointer:0,wrongFinished:true,finished:true});
        }catch(_){}
        await showFinal(); return;
      }
      await persistWrong({wrongModeActive:true,wrongList:WRONG.slice(),wrongPointer:WRP,wrongFinished:false,finished:false});
  ...
  if(isVerify){
    ...
    refs.staticText.textContent=isFullyCorrect?STATIC_CORRECT[pickNonRepeat(STATIC_CORRECT,"correct")]||"Corect":"Gresit";
    ...
    Promise.all([
      ...
      WRMODE?persistWrong({wrongModeActive:true,wrongList:WRONG.slice(),wrongPointer:WRP,wrongFinished:false,finished:false}):Promise.resolve()
    ]).catch(err=>{
...
/***** WRONG MODE – varianta robustă + PERSIST *****/
...
async function startWrongModeSafe(){
  ...
  // 1) Înghețăm UI ca să nu flasheze ultima întrebare
  if(container){
  ...
  try{
    // 2) Construim lista wrong (preferăm cea precomputată în showFinal)
    ...
    // 3) Prefetch puternic (inclusiv imaginea primei întrebări)
    ...
    // 4) Activăm WRONG MODE + persist session
    WRMODE=true; WRONG=list.slice(0); WRP=0;
    await persistWrong({wrongModeActive:true,wrongList:WRONG.slice(),wrongPointer:0,wrongFinished:false,finished:false});

    pruneBUF();

    // 5) Populate atomic, apoi decongelăm
    SEL=[]; ANSW=false;
    await updateInPlace(WRONG[WRP],{resetSelections:true});
    await prefetchAhead(WRONG[WRP]||0);

  }catch(e){
    console.error("EROARE la wrong-mode:",e);
...
D.addEventListener("click",async e=>{
  ...
  if(e.target.closest(".next-chapter-btn")){
    vibrateShort(); stopActiveMedia();
    const next=await getNextChapterSlug();
...
/* Idempotent: segmente bară finală dacă lipsesc */
