// SINGLE Webflow.push – tot JS-ul într-un singur bloc
...
const activeWrap = ()=> (WA.classList.contains(ACTIVE)?WA:WB);
const bufferWrap = ()=> (WA.classList.contains(ACTIVE)?WB:WA);
...
const legacyPercentEl=$(".chapter-final-screen .chapter-progress-percent"); if(legacyPercentEl) legacyPercentEl.style.display="none";

/* ===== Progress bar final helpers ===== */
...
function applyNoImageState(refs,on){
  // IMPORTANT: aplicăm pe WRAPPER-ul curent, nu pe părintele comun
...
/* ===== AI CHAT label (forțat) + butoane ===== */
function ensureAIChatLabel(btn){
  if(!btn) return;
  btn.textContent="AI CHAT";              // forțează label indiferent de Webflow
  ...
  refs.accHeader.addEventListener("click",async()=>{
    ...
    if(opening){
      if(c){
        ...
        try{ const imgs=[...c.querySelectorAll("img")]; if(imgs.length) await Promise.all(imgs.map(im=>im.decode?im.decode():Promise.resolve())).catch(()=>{});}catch(_){}
...
let REFS=collectRefs(activeWrap());
...
if(FIN){
  ...
  // showFinal va seta și progresul grafic
  // (apelăm mai jos după definire)
}else{
...
async function showFinal(){
  try{
    ...
    if(chapterTextFinal){ const chDoc=await firebase.firestore().collection("mediu_invatare_chapters").doc(CHAP).get(); if(chDoc.exists) chapterTextFinal.textContent=chDoc.data().Name; }
  }catch(_){}
...
/* ===== Pornire final, dacă era FIN ===== */
if(FIN){ await showFinal(); }


/* ====== Interacțiuni principale (Verify / Next / Finish) ====== */
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isNext && ANSW){
    // PRE-RENDER în buffer pentru a evita flicker-ul în UI
    if(WRMODE){
      WRP++;
      if(WRP>=WRONG.length){
  ...
  if(isVerify){
    ...
    // Scroll la explicații (dacă există)
...
/* ====== Alte acțiuni globale (reset, next chapter, etc.) ====== */
...
/* ====== Boot final (dacă era FIN înainte de randare) ====== */
