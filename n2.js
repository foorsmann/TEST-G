...
/* ===============================
   MULTI-SET BUTTONS (desktop + mobil)
   =============================== */
...
function getAllButtonsWraps(){
  const root=QSEC||D;
  return Array.from(root.querySelectorAll(".submit-next-buttons, .submit-next-buttons-mobile"));
}
function pickVisibleButtonsWrap(){
  const all=getAllButtonsWraps();
  return all.find(isVisible) || all[0] || null;
}
function getSubmitBtns(){
  const root=QSEC||D;
  return Array.from(root.querySelectorAll(".submit-btn"));
}
function getExplainBtns(){
  const root=QSEC||D;
  return Array.from(root.querySelectorAll(".explanation-btn, .explanation-card"));
}
function getAiBtns(){
  const root=QSEC||D;
  return Array.from(root.querySelectorAll(".ask-ai-btn, .ai-pop-wrapper"));
}
function setSingleBtnState(on){
  getAllButtonsWraps().forEach(w=>w.classList.toggle("single-btn-state",!!on));
}
function setSubmitStateAll(state){
  getSubmitBtns().forEach(btn=>{
...
function ensureAIChatLabel(el,force=false){
  if(!el) return;
  const t=(el.textContent||"").trim(); if(force||!t) el.textContent="AI CHAT";
  el.setAttribute("aria-label","AI CHAT"); el.setAttribute("role","button");
}
function ensureAIChatLabelAll(force=false){ getAiBtns().forEach(b=>ensureAIChatLabel(b,force)); }
...
/* HYDRATE + acordion (modificat pentru multi-set) */
function hydrateExistingDom(){
  ...
  // <<< nou: alege containerul vizibil (desktop sau mobil)
  refs.buttonsWrap = pickVisibleButtonsWrap(); if(!refs.buttonsWrap) return false;
  ...
  // Nu mai legăm strict pe un singur set; folosim delegare în QSEC.
  ensureAIChatLabelAll(true);
  return true;
}
...
/* ===== rebind dinamic pe setul vizibil (și legări dacă lipsesc) ===== */
function rebindButtonsWrapIfNeeded(){
  const w=pickVisibleButtonsWrap();
  if(!w) return;
  if(w===refs.buttonsWrap){
    // totuși actualizează submitBtn în cazul în care a fost recreat
    refs.submitBtn = w.querySelector(".submit-btn") || refs.submitBtn;
    refs.explainBtn= w.querySelector(".explanation-btn")||w.querySelector(".explanation-card")||refs.explainBtn;
    refs.aiBtn     = w.querySelector(".ask-ai-btn")||w.querySelector(".ai-pop-wrapper")||refs.aiBtn;
    return;
  }
  refs.buttonsWrap=w;
  refs.submitBtn = w.querySelector(".submit-btn") || refs.submitBtn;
...
function setButtonsTransitionLock(on){
  transitioning=!!on;
  // aplică pe TOATE butoanele de submit
  getSubmitBtns().forEach(b=>{
    b.setAttribute("aria-disabled",on?"true":"false");
    b.classList.toggle("is-disabled",on);
  });
  // blochează interacțiunile pe ambele containere
  getAllButtonsWraps().forEach(w=>{
...
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  // ascunde explicatia + AI pana la verificare
  getExplainBtns().forEach(b=>b.style.display="none");
  getAiBtns().forEach(b=>b.style.display="none");
  ...
  setSingleBtnState(true); setSubmitStateAll("verify");
  ...
  /* restaurare din Firestore (normal sau wrong-mode în aceeași sesiune) */
  try{
    ...
    if(snap.exists){
      ...
      if(allowRestore && selSaved.length>0){
        ...
        getExplainBtns().forEach(b=>b.style.display="flex");
        getAiBtns().forEach(b=>b.style.display="flex");
        ensureAIChatLabelAll(true);
        setSingleBtnState(false);
        const lastWrong=WRMODE&&(WRP===WRONG.length-1),lastNormal=!WRMODE&&(CUR===TOTAL);
        setSubmitStateAll((lastWrong||lastNormal)?"finish":"next");
      }
    }
...
// rebind la schimbare de viewport (mobil/desktop)
window.addEventListener("resize", rebindButtonsWrapIfNeeded, {passive:true});
...
/***** SUBMIT / EXPLICATIE / AI — delegare pe .question-section + answeredAtMs pentru restaurare *****/
QSEC?.addEventListener("click",async(e)=>{
  // 1) EXPLICATIE
  const exBtn=e.target.closest(".explanation-btn, .explanation-card");
  if(exBtn){
    vibrateShort();
    rebindButtonsWrapIfNeeded();
    // Toggle ca în versiunea originală
    const opening=(exBtn && exBtn.innerHTML.indexOf("<svg")===-1);
    ...
    if(opening){
      ...
      exBtn.innerHTML=`<svg width="15" height="15" viewBox="0 0 32 32"><path d="M16 10l10 10H6z" fill="#fff"/></svg>`;
  ...
  // 2) AI CHAT
  const aiBtn=e.target.closest(".ask-ai-btn, .ai-pop-wrapper");
  if(aiBtn){
    vibrateShort();
    rebindButtonsWrapIfNeeded();
    const v=refs.videoWrap?.querySelector("video");
  ...
  if(isVerify){
    if(SEL.length===0){
      ...
      getSubmitBtns().forEach(b=>b.style.gridColumn="1 / 2");
      return;
    }
...
D.addEventListener("keydown",e=>{
  if(e.key==="Enter"){
    const visBtn = getSubmitBtns().find(isVisible) || refs.submitBtn;
    if(visBtn){ e.preventDefault(); visBtn.click(); }
