...
/* ====== BUTOANE MOBILE: sincronizare & helper-e ====== */
const refs={
...
const allSubmitBtns  = () => [refs.submitBtn,  refs.submitBtnMobile].filter(Boolean);
const allExplainBtns = () => [refs.explainBtn, refs.explainBtnMobile].filter(Boolean);
const allAiBtns      = () => [refs.aiBtn,     refs.aiBtnMobile].filter(Boolean);
const allBtnWraps    = () => [refs.buttonsWrap, refs.buttonsWrapMobile].filter(Boolean);

function mirrorClass(srcEl, dstEl, cls){
  if(!srcEl || !dstEl) return;
  const apply=()=>dstEl.classList.toggle(cls, srcEl.classList.contains(cls));
  apply();
  new MutationObserver(apply).observe(srcEl,{attributes:true,attributeFilter:["class"]});
}

function setSubmitState(_btn,state){
  const apply=(btn,isDesktop)=>{
    if(!btn) return;
    ...
    switch(state){
      case "verify":
        ...
        if(isDesktop) btn.style.gridColumn="1 / 2";
        else btn.style.removeProperty("grid-column");
        break;
      case "next":{
        const last=WRMODE?(WRP===WRONG.length-1):(CUR===TOTAL);
        btn.textContent=last?"FINALIZEAZA":"CONTINUA";
        if(last) btn.classList.add("is-finish","finish-chapter-btn");
        else btn.classList.add("is-next");
        if(isDesktop) btn.style.gridColumn="2 / 3";
        else btn.style.removeProperty("grid-column");
        break;
      }
      case "finish":
  ...
  apply(refs.submitBtn,true);
  apply(refs.submitBtnMobile,false);
}

function addSingleBtnState(){ allBtnWraps().forEach(w=>w?.classList.add("single-btn-state")); }
function removeSingleBtnState(){ allBtnWraps().forEach(w=>w?.classList.remove("single-btn-state")); }

function setButtonsTransitionLock(on){
  const disabled=!!on;
  allSubmitBtns().forEach(b=>{
    if(!b) return;
    b.setAttribute("aria-disabled",disabled?"true":"false");
    b.classList.toggle("is-disabled",disabled);
  });
  allBtnWraps().forEach(w=>{
    if(!w) return;
    if(disabled) w.style.setProperty("pointer-events","none","important");
    else w.style.removeProperty("pointer-events");
  });
}

const setExplainVisible=(on)=> allExplainBtns().forEach(b=> b && (on ? b.style.setProperty("display","flex","important") : (b.style.display="none")));
const setAiVisible     =(on)=> allAiBtns().forEach(b=>      b && (on ? b.style.setProperty("display","flex","important") : (b.style.display="none")));
function setExplainBtnUI(open){
  const openIcon = `<svg width="15" height="15" viewBox="0 0 32 32"><path d="M16 10l10 10H6z" fill="#fff"/></svg>`;
  allExplainBtns().forEach(b=>{
    if(!b) return;
    b.innerHTML = open ? openIcon : "EXPLICATIE";
  });
}

function ensureAIChatLabel(force=false){
  allAiBtns().forEach(btn=>{
    if(!btn) return;
    const t=(btn.textContent||"").trim(); if(force||!t) btn.textContent="AI CHAT";
    btn.setAttribute("aria-label","AI CHAT"); btn.setAttribute("role","button");
  });
}
...
function hydrateExistingDom(){
  ...
  // ține wrap-urile sincronizate pe clasa single-btn-state (în plus facem și setări explicite în cod)
  if(refs.buttonsWrap && refs.buttonsWrapMobile){
    mirrorClass(refs.buttonsWrap, refs.buttonsWrapMobile, "single-btn-state");
  }
  ...
  // Explain buttons (desktop + mobile)
  [...allExplainBtns()].forEach(btn=>{
    if(!btn || btn.dataset.boundExp) return;
    btn.addEventListener("click",()=>{
      vibrateShort();
      const opening = (btn.innerHTML.indexOf("<svg")===-1);
...
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  addSingleBtnState(); setSubmitState(refs.submitBtn,"verify");
  ...
  try{
    ...
    if(snap.exists){
      ...
      if(allowRestore && selSaved.length>0){
        ...
        refs.feedbackWrap.style.setProperty("display","flex","important"); setExplainVisible(true); setAiVisible(true); ensureAIChatLabel(true);
        removeSingleBtnState();
...
/***** SUBMIT / NEXT / FINISH – cu answeredAtMs pentru restaurare *****/
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    ...
    applyLetterStatesByCorrect(refs.qEl,cor);
    clearWrongCards(refs.qEl);   
    applyWrongCardsByCorrect(refs.qEl,cor);

    const selected=SEL.slice().sort().join(","), 
          correct =cor.slice().sort().join(","), 
    ...
    refs.correctAns.textContent=ord.join(", ");
...
D.addEventListener("keydown",e=>{
  if(e.key==="Enter"){ 
    const btn = allSubmitBtns().find(b=>b && !b.classList.contains("is-disabled"));
