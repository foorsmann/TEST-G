...
/* --- Helper sigur pentru selecții locale + globale (fără `||`) --- */
function pick(sel,wrap){ return (wrap&&wrap.querySelector(sel)) || D.querySelector(sel) || null; }

/* === collectRefs – refăcut fără `||` ca să nu mai poată apărea eroarea cu '|' === */
function collectRefs(wrapper){
  const refs={
    root:wrapper,
    qEl: wrapper.closest(".questions-collection-list-item") || wrapper,
    qWrap:wrapper,
    anchorTop: pick(".explanation-anchor-top",wrapper),
    qText: pick(".question-text",wrapper),
    progress: pick(".progress-counter",wrapper),
    mainWrap: pick(".main-content-wrapper",wrapper),
    optWrap: pick(".options-wrapper",wrapper),
    cards: Array.from(wrapper.querySelectorAll(".option-card")),
    items:[],
    imgWrap: pick(".question-image-wrapper",wrapper),
    img: pick(".question-image",wrapper),
    videoWrap: pick(".explanation-video-wrapper",wrapper),
    acc: pick(".custom-accordion",wrapper),
    accHeader:null,accArrow:null,accContent:null,accBody:null,expLabelEl:null,accAnchor:null,
    feedbackWrap: pick(".feedback-wrapper",wrapper),
    staticText:null,correctAns:null,
    buttonsWrap: pick(".submit-next-buttons",wrapper),
    submitBtn:null,explainBtn:null,aiBtn:null,
    anchor: pick(".explanation-anchor",wrapper)
  };
...
/* === buton AI CHAT – forțăm eticheta indiferent de textul Webflow === */
...
function bindAccordion(refs){
  if(!refs.acc||!refs.acc.querySelector) return;
  if(refs.acc.dataset.boundAcc==='1') return;
  refs.accHeader=refs.acc.querySelector(".custom-accordion-header");
  refs.accArrow =refs.acc.querySelector(".custom-accordion-arrow");
  refs.accContent=refs.acc.querySelector(".custom-accordion-content");
  refs.accBody  =refs.acc.querySelector(".custom-accordion-body");
...
function bindAiBtn(refs){
  ...
  refs.aiBtn.addEventListener("click",()=>{ 
    ...
    ensureAIPop().style.display="flex"; 
...
/* === renderer întrebări cu anti-flicker (lock min-height + A/B buffer) === */
...
/* === flip logic & final screen helpers === */
const ACTIVE="is-active";
...
async function showFinal(){
  try{
    const ct=$(".chapter-text-final");
    if(ct){ const chDoc=await firebase.firestore().collection("mediu_invatare_chapters").doc(CHAP).get(); if(chDoc.exists) ct.textContent=chDoc.data().Name; }
  }catch(_){}
...
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    ...
    requestAnimationFrame(()=>{
      const scroller=refs.root?.querySelector(".question-wrapper-inner")||getScrollParent(refs.root);
      const anchor=refs.root.querySelector(".explanation-anchor")||pick(".explanation-anchor",refs.root);
      if(anchor) {
        ...
        const to=(isWin?window.pageYOffset:scroller.scrollTop)+top;
