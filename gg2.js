...
/* === Scroll helper (FIX: folosește 'dur', nu 'd') === */
...
function scrollContainerToAnchor(target,container,{behavior="smooth",offset=0,delay=0}={}){
  ...
  const step=ts=>{
    ...
    const y=Math.round(start+dist*easeInOut(p));
    isWin?window.scrollTo(0,y):c.scrollTop=y;
...
function bindSelection(refs){
  refs.cards.forEach(card=>{
    ...
    card.addEventListener("click",()=>{
      if(READ_ONLY||ANSW) return; // READ_ONLY definit în Partea 2
      vibrateShort();
...
async function acquireLock({force=false}={}){
  try{
    ...
    console.warn("acquireLock err:",e); applyReadOnlyUI(true);
  }
}
