...
/* ===== IMAGINE NON-BLOCKING: fără așteptare la decodare ===== */
function seamlessSetImage(refs,url){
...
/* ===== No-image state pe WRAPPERUL curent, ca să nu mai „sară” layout-ul ===== */
...
/* ===== Restore răspuns din Firestore, ASINCRON (nu blochează flip-ul) ===== */
function restoreSavedAnswerAsync(refs,idx){
  if(WRMODE) return; // în wrong-mode vrem răspuns proaspăt
  ansCollection.doc(`${CHAP}_${idx}`).get().then(snap=>{
    if(!snap.exists) return;
    const d=snap.data(), selSaved=(d.selected||[]).map(x=>String(x||"").toUpperCase()), corSaved=(d.correct||[]).map(x=>String(x||"").toUpperCase());
    if(selSaved.length===0) return;
    ...
    if(refs.aiBtn){ refs.aiBtn.style.setProperty("display","flex","important"); ensureAIChatLabel(refs.aiBtn); }
  ...
  }).catch(e=>console.warn("Restore async error:",e));
}
/* ===== RENDER întrebarea în wrapper (non-blocking img, restore opțional) ===== */
async function renderQuestionInWrapper(refs,idx,{resetSelections=false,skipRestore=false}={}){
  ...
  // IMAGINE non-blocking (fără await)
  if(q.Image){ seamlessSetImage(refs,q.Image); } else { seamlessSetImage(refs,""); }
  // RESTORE asyncron (opțional – pentru pre-render îl sărim)
  if(!skipRestore) restoreSavedAnswerAsync(refs,idx);
...
if(FIN){
  ...
  revealNow();
  /* === PRE-RENDER pe buffer pentru întrebarea URMĂTOARE (non-blocking, fără restore) === */
  (function preRenderNextOuter(){
    const nextIdx = WRMODE ? (WRONG[WRP+1]) : (CUR<TOTAL?CUR+1:null);
    if(!nextIdx) return;
    const buf=getBufferWrap(), bufRefs=collectRefs(buf);
...
function ensureAIPop(){
  ...
  /* --- Helpers pentru flip & refs --- */
  function activeWrap(){ return getActiveWrap(); }
  function bufferWrap(){ return getBufferWrap(); }
  ...
  /* --- Pre-render pe buffer pentru următoarea întrebare (non-blocking, fără restore) --- */
  function schedulePreRenderNext(){
    const nextIdx = WRMODE ? (WRONG[WRP+1]) : (CUR<TOTAL?CUR+1:null);
    if(!nextIdx) return;
    const buf=bufferWrap(), bufRefs=collectRefs(buf);
    buf.dataset.prerenderIdx=String(nextIdx);
    renderQuestionInWrapper(bufRefs,nextIdx,{resetSelections:true,skipRestore:true});
    // ținem cald și cache-ul logic
    prefetchAhead(WRMODE?WRONG[WRP]:CUR);
  }

  /* --- Flip instant: folosește bufferul dacă e deja pre-rendat pentru idx --- */
  async function flipToIndex(idx,{resetSelections=false}={}){
    ...
    // oprim media din activ (fără să blocăm)
    stopActiveMedia(actRefs);

    const preReady = (buf.dataset.prerenderIdx===String(idx));
    if(!preReady){
      // pregătim bufferul acum (tot non-blocking pentru imagine & fără restore)
    ...
    // după flip facem restore-ul răspunsului salvat (dacă există)
    restoreSavedAnswerAsync(refsOf(activeWrap()), idx);

    // programăm pre-render pentru următoarea întrebare
  ...
  async function showFinal(){
    ...
    // pregătim lista Wrong în fundal pentru "Refă doar greșite"
    try{
  ...
  /* --- Click handler pentru butonul principal (Verify/Next/Finish) --- */
  ...
  // dacă erai deja finalizat din DB, afișăm sumarul
