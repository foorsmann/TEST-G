...
if(persistedWM && persistedWM.active && (persistedWM.list||[]).length){
  ...
  /* rendering se face în updateInPlace/AB-fill mai jos */
  revealNow();
}else if(FIN){
  await showFinal(); D.body.classList.remove("page-loading");
}else{
...
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  if(IS_AB){
    ...
    /* marchează vizibil next, scoate prev */
    next.classList.add("visible");
    prev.classList.remove("visible");
    /* finalizează tranziția și swap referințe */
    AB.swap();                 /* acum AB.current === next */
    hydrateInto(AB.current);   /* refs pointează către activ */
  }else{
    /* single: doar scoatem preparing */
    // no-op
  }
