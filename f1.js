...
/* === Redirect instant pe device-uri inactive (minim writes/reads) ===
   - Beacon doar pe tab/device activ (focus + vizibil)
   - Listener se atașează DOAR când tabul NU e activ (hidden sau fără focus)
   - Dacă pe alt device vine ping proaspăt -> redirect imediat pe acest device
*/
const CLIENT_ID=(()=>{try{
  let id=localStorage.getItem("clientId");
  if(!id){ id=(crypto.randomUUID?.()||(""+Date.now()+Math.random().toString(16).slice(2))); localStorage.setItem("clientId",id); }
  return id;
}catch(_){return(""+Date.now()+Math.random().toString(16).slice(2));}})();

const PRESENCE_REF=db.collection("users").doc(user.uid).collection("presence").doc("quiz");
let beaconTimer=null, presenceUnsub=null;
const REDIRECT_URL="https://webarcs-ultra-awesome-site.webflow.io/mediu-invatare";
const FRESH_MS=12000; // 12s „proaspăt”
function tabIsActive(){ return !D.hidden && D.hasFocus(); }
async function sendBeaconOnce(){ try{
  await PRESENCE_REF.set({activeSessionId:CLIENT_ID,activePingAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});
}catch(_){/* best-effort */} }
function startBeacon(){
  if(beaconTimer||!tabIsActive()) return;
  sendBeaconOnce();
  beaconTimer=setInterval(sendBeaconOnce,15000);
}
function stopBeacon(){
  if(beaconTimer){clearInterval(beaconTimer); beaconTimer=null;}
}
function attachPresenceListenerIfInactive(){
  if(presenceUnsub){ presenceUnsub(); presenceUnsub=null; }
  if(tabIsActive()) return; // nu ascultăm când e activ => zero citiri inutile
  presenceUnsub=PRESENCE_REF.onSnapshot(snap=>{
    const d=snap.data()||{};
    const mine=d.activeSessionId===CLIENT_ID;
    const ts=d.activePingAt && d.activePingAt.toMillis ? d.activePingAt.toMillis() : 0;
    const age=Date.now()-ts, fresh=age>=0 && age<FRESH_MS;
    if(!mine && fresh){
      // Suntem inactivi și alt device e activ acum -> redirect instant
      try{ location.replace(REDIRECT_URL); }catch(_){ location.href=REDIRECT_URL; }
    }
  });
}
["visibilitychange","focus","blur","pageshow","pagehide"].forEach(evt=>{
  W.addEventListener(evt,()=>{ tabIsActive()? (startBeacon(), attachPresenceListenerIfInactive()) : (stopBeacon(), attachPresenceListenerIfInactive()); },{passive:true});
});
/* Pornește cu starea curentă */
tabIsActive()? startBeacon() : attachPresenceListenerIfInactive();

/* === Restul inițializărilor UI === */
...
function collectRefs(wrapper){
  ...
  const refs={
    ...
    progress:need(".progress-counter") || $(".progress-counter"), /* ← FIX: era `| |` */
...
/* === Imagine & state no-image (anti-flicker) === */
...
/* === Randare întrebare (cu anti-flicker la trecerea imagine->text) === */
...
/* === Flux inițial (final sau întrebare curentă) === */
if(FIN){
  if(chapterTextFinal) chapterTextFinal.textContent=chapterName;
  await (async function(){ FINAL?.style.setProperty("display","flex","important"); if(QSEC) QSEC.style.setProperty("display","none","important"); })();
  D.body.classList.remove("page-loading");
}else{
        ...
        if(window.lottie){
          ...
          try{
            window.lottie.loadAnimation({
              container:lottieEl,renderer:"svg",loop:false,autoplay:true,
              path:"https://cdn.prod.website-files.com/68589124f5ef093107d3fdc2/6868a90be26405b0d160eb81_Main%20Scene%20(1).json"
            });
          }catch(_){}
        }
      }
...
D.addEventListener("click",async e=>{
  ...
  if(e.target.closest(".next-chapter-btn")){
    ...
    const i=arr.indexOf(CHAP); const next=(i!==-1&&i<arr.length-1)?arr[i+1]:null;
  ...
  if(e.target.closest(".back-to-chapter-final-screen")){
    const aw=$(".alert-wrapper"); if(aw) aw.style.display="none";
    FINAL?.style.setProperty("display","flex","important");
    return;
  }
});
