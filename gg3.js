...
try{
  ...
  if(!fs._settingsFrozen){
    fs.settings({ignoreUndefinedProperties:true,experimentalAutoDetectLongPolling:true,useFetchStreams:false});
  }
}catch(_){}
...
function bindSelection(refs){
  refs.cards.forEach(card=>{
    ...
    card.addEventListener("click",()=>{ 
      if(READ_ONLY||ANSW) return; // READ_ONLY e definit în Partea 2
      vibrateShort();
...
const SESSION_ID=(()=>{const s=(window.crypto&&crypto.getRandomValues)?crypto.getRandomValues(new Uint32Array(4)):[Math.random()*1e9,Math.random()*1e9,Math.random()*1e9,Math.random()*1e9];return 's-'+Date.now().toString(36)+'-'+Array.from(s).map(x=>Math.floor(x).toString(36)).join('');})();
...
function ensureSessionBanner(){
  ...
  if(!b){
    ...
    b.innerHTML=`<span class="sb-text">Capitolul e deschis activ pe alt dispozitiv. Ești în modul vizualizare.</span><button class="sb-btn sb-take">Preia controlul</button>`;
...
async function acquireLock({force=false}={}){
  try{
    const res=await db.runTransaction(async tx=>{
      ...
      if(!snap.exists){
        tx.set(sessRef,{
          ownerId:SESSION_ID,ownerUid:user.uid,chapterSlug:CHAP,
          ownerAgent:navigator.userAgent||"",
          ownerSince:firebase.firestore.FieldValue.serverTimestamp(),
      ...
      if(force||stale||d.ownerId===SESSION_ID||d.ownerUid===user.uid&&d.ownerId===SESSION_ID){
        tx.set(sessRef,{
          ownerId:SESSION_ID,ownerUid:user.uid,chapterSlug:CHAP,
          ownerAgent:navigator.userAgent||"",
          lastHeartbeat:firebase.firestore.FieldValue.serverTimestamp()
    ...
    if(res.owned){ applyReadOnlyUI(false); startHeartbeat(); } else { applyReadOnlyUI(true); stopHeartbeat(); }
...
function watchSession(){
  ...
  },err=>{ console.warn("watchSession error:",err); });
}
...
/* ==== INITIALIZARE: lock -> progres -> render -> subscribe ==== */
