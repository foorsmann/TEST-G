...
/* ===== State (fără redeclarări globale) ===== */
let BUF={},SEL=[],ANSW=false; let CUR=1,FIN=false; let WRONG=[],WRMODE=false,WRP=0;
let WRONG_PREPARED=[],WRONG_PREPARED_READY=false; /* <-- eliminat START_WRONG_IN_PROGRESS aici */
let WRONG_STARTED_AT_MS=0;
/* Flag global sigur anti-redeclarări */
function getWrongStartFlag(){ return !!window.__AT_WRONG_IN_PROGRESS; }
function setWrongStartFlag(v){ window.__AT_WRONG_IN_PROGRESS = !!v; }
...
const refs={
  scope:null,qEl:null,qWrap:null,qText:null,progress:null,mainWrap:null,
...
function hydrateInto(scopeRoot){
  ...
  refs.progress = (QSEC||D).querySelector(".progress-counter");
  ...
  refs.feedbackWrap = (QSEC||D).querySelector(".feedback-wrapper");
...
async function startWrongModeSafe(){
  if(getWrongStartFlag()) return;
  setWrongStartFlag(true);
...
/* ===== Initial render efectiv (după prefetch/reveal) ===== */
