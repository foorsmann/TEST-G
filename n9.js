...
/* ==== NEW: helpers desktop+mobile ==== */
const both = {
  get wraps(){ return [refs.buttonsWrap, refs.buttonsWrapMobile].filter(Boolean); },
  get submits(){ return [refs.submitBtn, refs.submitBtnMobile].filter(Boolean); },
  get explains(){ return [refs.explainBtn, refs.explainBtnMobile].filter(Boolean); },
  get ai(){ return [refs.aiBtn, refs.aiBtnMobile].filter(Boolean); }
};
function setSubmitStateBoth(state){
  setSubmitState(refs.submitBtn, state);
  // IMPORTANT: nu mai ating grid-ul mobile; doar schimb starea vizuală a textului
  setSubmitState(refs.submitBtnMobile, state);
}
/* DOAR DESKTOP! – pe mobile lăsăm Webflow să decidă gridul */
function setSingleBtnStateDesktop(on){
  if(refs.buttonsWrap){
    refs.buttonsWrap.classList.toggle("single-btn-state", !!on);
  }
  if(on && refs.submitBtn){
    refs.submitBtn.style.gridColumn="1 / 2";
  }
}
function ensureAIChatLabel(btn){
  if(!btn) return;
  const t=(btn.textContent||"").trim();
  if(!t) btn.textContent="AI CHAT";
  btn.setAttribute("aria-label","AI CHAT");
  btn.setAttribute("role","button");
}
function ensureAIChatLabelAll(){
  ensureAIChatLabel(refs.aiBtn);
  ensureAIChatLabel(refs.aiBtnMobile);
}

/* === utilitare pentru opțiuni === */
...
/* ===== Explicații: stare comună desktop+mobile ===== */
...
/* HYDRATE + acordion + butoane desktop&mobile */
function hydrateExistingDom(){
  ...
  /* Mobile buttons (mirror) — NU modificăm layout-ul; îl controlezi din Webflow */
  ...
  /* Etichete corecte pentru AI pe ambele butoane */
  ensureAIChatLabelAll();
  ...
  /* Accordion press feedback + toggle */
  ...
  /* Toggle Explicație (desktop + mobile sincron) */
...
/* === restaurare stare “verificată” și în wrong-mode (numai încercările curente) === */
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  /* ascundem explicatia + AI până la verificare – pe mobile nu atingem gridul */
  ...
  /* DOAR desktop: un singur buton în stare VERIFY */
  setSingleBtnStateDesktop(true);
  ...
  /* restaurare din Firestore (cu condițiile wrong-mode) */
  try{
    ...
    if(snap.exists){
      ...
      if(allowRestore && selSaved.length>0){
        ...
        ensureAIChatLabelAll();
        /* la răspuns restaurat: revenim la starea cu toate trei butoanele vizibile */
        setSingleBtnStateDesktop(false);
...
QSEC?.addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    if(SEL.length===0){
      ...
      playOne(alertSound);
      /* DOAR desktop: păstrăm un singur buton */
      setSingleBtnStateDesktop(true);
      if(refs.submitBtn) refs.submitBtn.style.gridColumn="1 / 2";
      return;
    }
    ...
    /* ARĂTAM explicatia + AI după verificare (desktop + mobile). Nu atingem layout-ul mobile. */
    if(refs.explainBtn) refs.explainBtn.style.setProperty("display","flex","important");
    if(refs.explainBtnMobile) refs.explainBtnMobile.style.setProperty("display","flex","important");
    if(refs.aiBtn) refs.aiBtn.style.setProperty("display","flex","important");
    if(refs.aiBtnMobile) refs.aiBtnMobile.style.setProperty("display","flex","important");
    ...
    /* desktop: revenim la layout-ul cu toate trei butoanele vizibile */
    setSingleBtnStateDesktop(false);
...
function primarySubmitBtn(){
  // alege butonul vizibil (mobile are prioritate pe ecrane mici)
