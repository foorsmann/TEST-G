...
/* ==== NEW: helpers pentru sincronizare desktop + mobile ==== */
const both = {
...
function setSingleBtnState(on){
  (both.wraps).forEach(w=>{
    if(!w) return;
    w.classList.toggle("single-btn-state", !!on);
  });
  if(on){
    (both.submits).forEach(b=>{ if(b) b.style.gridColumn="1 / 2"; });
  }
}
function ensureAIChatLabel(btn){
  if(!btn) return;
  const t=(btn.textContent||"").trim(); if(!t) btn.textContent="AI CHAT";
...
/* ===== Explicații: stare comună, sincronizăm ambele butoane ===== */
...
async function updateInPlace(idx,{resetSelections=false}={}){
  ...
  /* hide both explain & AI until verificare */
  ...
  renderExplainBtnUI(false); /* sync both explain buttons to closed */
  ...
  setSingleBtnState(true); setSubmitStateBoth("verify");
