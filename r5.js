...
/* ===== Double buffer: .question-wrapper-A / .question-wrapper-B (switch instant) ===== */
const QSEC=$(".question-section");
(function ensureABShell(){
  const first=$(".question-wrapper");
  if(!first) return;
  let shell=first.closest(".question-shell-w");
  if(!shell){
  ...
  // Marcați/poziționați A
  first.classList.add("question-wrapper-A");
  first.style.position="relative"; /* lane principală, B vine absolut */
  first.style.willChange="contents";

  // Clonați B dacă lipsește
  let second=$(".question-wrapper-B", shell);
  if(!second){
    second=first.cloneNode(true);
    second.classList.remove("visible");
    second.classList.add("question-wrapper-B","ab-hidden");
    second.style.position="absolute";
    second.style.inset="0";
    second.style.pointerEvents="none";
    shell.appendChild(second);
  }else{
    second.classList.add("ab-hidden");
    second.style.position="absolute";
    second.style.inset="0";
  }
})();

/* Helper pentru a afla wrapperul activ și bufferul (nu animăm opacitatea) */
function getAB(){
  const shell=$(".question-shell-w"); if(!shell) return null;
  const A=$(".question-wrapper-A",shell),B=$(".question-wrapper-B",shell);
  const active = A && !A.classList.contains("ab-hidden") ? A : B && !B.classList.contains("ab-hidden") ? B : A;
  const buffer = (active===A)?B:A;
  return {shell,A,B,active,buffer};
}

/* ===== UI / sunete / toast ===== */
const QWRAP=($(".question-shell-w")?.querySelector(".question-wrapper-A"))||$(".question-wrapper");
...
/* === SANITIZARE ACTIVĂ (deja patch-uit insertAdjacentHTML mai sus) === */
...
/* declarăm o singură dată acest trio; NU re-declarăm mai jos */
...
/* === PATCH critic: pruning BUF fără sintaxă exotică === */
function pruneBUF(){
  const keep=new Set();
  if(!WRMODE){
...
/* ====== REFS (folosite pe wrapperul activ) ====== */
let refs={
...
/* ===== HYDRATE DOM pe wrapperul ACTIV (A sau B) ===== */
function hydrateExistingDom(rootOverride){
  const root = rootOverride || (getAB()?.active) || (QSEC||document);
  ...
  /* mobil (NU atingem layoutul din Webflow) */
...
/* === utilitare opțiuni & explicații (folosite mai jos) === */
...
/* === helperi UI pentru butoanele desktop + mobil (declarații hoist-abile) === */
function applyAIChatText(){
  if(refs.aiBtn){
    refs.aiBtn.textContent="AI CHAT";
    refs.aiBtn.setAttribute("aria-label","AI CHAT");
    refs.aiBtn.setAttribute("role","button");
  }
  if(refs.aiBtnMobile){
    refs.aiBtnMobile.textContent="AI CHAT";
    refs.aiBtnMobile.setAttribute("aria-label","AI CHAT");
    refs.aiBtnMobile.setAttribute("role","button");
  }
}
...
/* === AI pop-up minimalist (idempotent) === */
let AIPop=null;
function ensureAIPop(){
  ...
  if(!AIPop){
    ...
    AIPop.addEventListener("click",e=>{ if(e.target.closest(".ask-ai-pop-up-close")||e.target===AIPop){ const v=document.querySelector(".ask-ai-pop-up"); if(v){ v.style.display="none"; document.body.style.overflow=""; } }});
    AIPop.querySelector(".audio-ai-btn")?.addEventListener("click",()=>{/* hook STT -> GPT -> TTS */});
  }
  return AIPop;
}

/* === corecte din întrebare === */
function parseCorrectLetters(q){
  return String(q["Correct Answers"]||"").split(",").map(x=>String(x||"").trim().toUpperCase()).filter(x=>x==="A"||x==="B"||x==="C");
}

/* === Update cu dublu-buffer: randăm în buffer apoi switch instant (fără opacitate) === */
async function updateInPlace(idx,{resetSelections=false}={}){
  const ab=getAB();
  const shell=ab?.shell;
  const active=ab?.active;
  const buffer=ab?.buffer||active;
  const q=BUF[idx]; 
  if(!buffer){ return; }
  if(!q){ await showFinal(); return; }

  /* blochează layout jumps cât timp pregătim buffer-ul */
  const base=active||buffer;
  const prevH=base.offsetHeight;
  base.classList.add("no-transition","is-preparing");
  if(prevH) base.style.minHeight=prevH+"px";

  /* scriem în buffer */
  const local=(sel,root=buffer)=>root.querySelector(sel);
  /* capitol + progres */
  const progressEl = local(".progress-counter");
  const chapterTxt = local(".chapter-text");
  if(chapterTxt) chapterTxt.textContent=sanitizeMaybe(chapterName);
  if(progressEl) progressEl.textContent = WRMODE ? `${pad2(WRP+1)}/${pad2(WRONG.length)}` : `${pad2(CUR)}/${pad2(TOTAL)}`;

  /* întrebare */
  const qTextEl = local(".question-text");
  if(qTextEl){ qTextEl.innerHTML=""; qTextEl.insertAdjacentHTML("afterbegin",sanitizeMaybe(q.Question||"")); }

  /* opțiuni */
  const cards = Array.from(buffer.querySelectorAll(".option-card"));
  const items = cards.map(c=>c.querySelector(".option-item"));
  const vals=[q["Option A"]||"", q["Option B"]||"", q["Option C"]||""];
  cards.forEach((card,i)=>{
    const it=items[i]; const val=vals[i]||"";
    if(it){ it.innerHTML=""; if(val) it.insertAdjacentHTML("afterbegin",sanitizeMaybe(val)); }
    card.style.display = val ? "" : "none";
  });

  /* explicație scrisă ascunsă până la verificare; video setăm după swap cu setVideoBlock() */
  const acc      = local(".custom-accordion");
  const accBody  = local(".custom-accordion-body");
  const accCont  = local(".custom-accordion-content");
  const videoWrap= local(".explanation-video-wrapper");
  if(acc){ acc.style.display="none"; acc.classList.remove("open"); }
  if(accCont){ accCont.style.maxHeight="0px"; }
  if(accBody){ accBody.innerHTML=""; const ex=q["Explanation"]||""; if(ex) accBody.insertAdjacentHTML("afterbegin",sanitizeMaybe(ex)); }
  if(videoWrap){ videoWrap.style.display="none"; videoWrap.innerHTML=""; }

  /* feedback reset */
  const fbWrap = local(".feedback-wrapper");
  const fbText = fbWrap?.querySelector(".correct-answer-static");
  const fbAns  = fbWrap?.querySelector(".correct-answer");
  if(fbWrap){ fbWrap.style.display="none"; }
  if(fbText){ fbText.textContent="Raspunsul corect este:"; }
  if(fbAns){ fbAns.textContent=""; }

  /* curățăm selecții/markeri în buffer */
  clearLetterStates(buffer); clearAllSelected(buffer); clearWrongCards(buffer);

  /* imagine (decode înainte) */
  try{ await (q.Image ? (q.__imgReady||ensureImageReadyFromQ(q)) : Promise.resolve()); }catch(_){}
  const imgEl = local(".question-image");
  const imgWrapEl = local(".question-image-wrapper");
  if(!q.Image){
    if(imgEl){ imgEl.removeAttribute("src"); imgEl.style.display="none"; imgEl.setAttribute("aria-hidden","true"); }
    if(imgWrapEl){ imgWrapEl.style.display="none"; }
    buffer.classList.add("no-image");
  }else{
    if(imgEl){
      imgEl.src=q.Image; imgEl.style.display=""; imgEl.removeAttribute("aria-hidden");
      imgEl.onerror=()=>{ imgEl.removeAttribute("src"); imgEl.style.display="none"; imgEl.setAttribute("aria-hidden","true"); buffer.classList.add("no-image"); };
    }
    if(imgWrapEl){ imgWrapEl.style.display=""; }
    buffer.classList.remove("no-image");
  }

  /* switch instant A/B: activ => hidden, buffer => activ */
  if(active && buffer && active!==buffer){
    active.classList.add("ab-hidden");
    buffer.classList.remove("ab-hidden");
  }

  /* re-hidrate refs pe NOUL activ */
  hydrateExistingDom(buffer);

  /* setăm video pe wrapperul activ via utilitarul global (Plyr etc.) */
  setVideoBlock(q["Explanation Video Link"]||"");

  /* reset selecții + stări butoane pe noul activ */
  SEL=[]; ANSW=false;
  ...
  /* restaurare răspuns (dar numai dacă e din aceeași sesiune în wrong-mode) */
  try{
  ...
  /* eliberează layout lock */
  requestAnimationFrame(()=>{
    base.classList.remove("no-transition","is-preparing");
    base.style.minHeight="";
  });
}

<!-- CONTINUARE din Partea 1/2, exact sub comentariul de acolo -->
/* === Final screen + pregătire WRONG prefetch === */
async function showFinal(){
  ...
  ensureFinalSegments(); ensureFinalPercentEl(); setFinalPercent("0%"); animateFinalProgress(cPct,wPct,ANIM_DUR);
...
/* asigurăm refs pe wrapperul activ curent (după bootstrap A/B) */
hydrateExistingDom(getAB()?.active);
...
/* === WRONG MODE – construire listă + start cu persistență === */
...
/* ATENȚIE: NU redeclarăm variabila aici! Folosim pe cea definită în blocul de “State”. */
async function startWrongModeSafe(){
  ...
  const ab=getAB(); const base=(ab?.active)||document.body;

  if(base){
    const h=base.offsetHeight;
    base.classList.add("no-transition","is-preparing");
    if(h) base.style.minHeight=h+"px";
  }
  ...
  try{
    ...
    if(!list.length){
      ...
      FINAL?.style.setProperty("display","flex","important");
      if(base){ base.classList.remove("no-transition","is-preparing"); base.style.minHeight=""; }
...
/* === Delegări ecran final / controale auxiliare === */
...
/* === QoL: ENTER = primary submit; ESC = închide pop/loader === */
