...
async function prefetchAhead(current){
  ...
  const targets=[current+1]; if(fast) targets.push(current+2);
...
/* ====== FIX: scope no-image doar pe WRAPPER ====== */
...
<!-- PARTEA 2 – continuare din <script> deja deschis -->
  /* ===== A/B flip utils & FINAL logic ===== */
  ...
  async function showFinal(){
    ...
    if(resetWrongBtn){ resetWrongBtn.style.display=hasWrong?"flex":"none"; }

    try{
      const pool=[]; answersSnap.forEach(doc=>{
        const d=doc.data(),sel=(d.selected||[]).slice().sort().join(","),cor=(d.correct||[]).slice().sort().join(",");
        if(sel&&sel!==cor){
          let idx=d.index;
  ...
  QSEC?.addEventListener("click",async(e)=>{
    ...
    if(isVerify){
      ...
      const selected=SEL.slice().sort().join(","), correct=cor.slice().sort().join(","), ok=selected.length>0&&selected===correct;
      refs.staticText&&(refs.staticText.textContent=ok?pickNonRepeat(STATIC_CORRECT,"correct"):pickNonRepeat(STATIC_WRONG,"wrong"));
  ...
  /* ===== if already finished ===== */
  if(typeof FIN!=="undefined" && FIN){ try{ await showFinal(); }catch(_){} }
}); // end Webflow.push
