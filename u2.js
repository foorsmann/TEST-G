/* ====== MINI-UTIL: escape/safe text pentru sanitizare țintită ====== */
(function(){
  function safeTextHTML(raw){ const d=document.createElement('div'); d.textContent=String(raw==null?'':raw); return d.innerHTML; }

  document.addEventListener('DOMContentLoaded',function(){
    const root=document.querySelector('.question-section')||document;

    // 1) Asigurăm feedback minim ca să nu pice hidratarea
    (function ensureFeedback(){
      let wrap=root.querySelector('.feedback-wrapper, .answer-feedback, .result-feedback');
      if(!wrap){
        const host=root.querySelector('.question-wrapper, .question-item, .question')||root;
        wrap=document.createElement('div'); wrap.className='feedback-wrapper';
        const s=document.createElement('div'); s.className='correct-answer-static'; s.textContent='Răspunsul corect este:';
        const a=document.createElement('div'); a.className='correct-answer';
        wrap.appendChild(s); wrap.appendChild(a);
        const anchor=root.querySelector('.explanation-anchor, .explain-anchor');
        if(anchor&&anchor.parentElement) anchor.parentElement.appendChild(wrap);
        else host.appendChild(wrap);
      }else{
        if(!wrap.querySelector('.correct-answer-static')){ const s=document.createElement('div'); s.className='correct-answer-static'; s.textContent='Răspunsul corect este:'; wrap.appendChild(s); }
        if(!wrap.querySelector('.correct-answer')){ const a=document.createElement('div'); a.className='correct-answer'; wrap.appendChild(a); }
      }
    })();

    // 2) A11y pentru butoanele non-<button> (Explain/AI, desktop + mobil)
    (function ensureButtonsA11y(){
      const sels=[
        '.submit-next-buttons .explanation-btn, .submit-next-buttons .explanation-card, .submit-next-buttons .explain-btn',
        '.submit-next-buttons .ask-ai-btn, .submit-next-buttons .ai-pop-wrapper, .submit-next-buttons .ai-btn',
        '.submit-next-buttons-mobile .explanation-btn-mobile, .submit-next-buttons-mobile .explanation-btn, .submit-next-buttons-mobile .explain-btn',
        '.submit-next-buttons-mobile .ask-ai-btn-mobile, .submit-next-buttons-mobile .ask-ai-btn, .submit-next-buttons-mobile .ai-btn'
      ];
      const force=(el)=>{
        if(!el) return;
        if(el.tagName.toLowerCase()!=='button'){
          ...
          if(!el.getAttribute('aria-label')&&!el.textContent.trim()){
            if(el.className.includes('explain')) el.setAttribute('aria-label','Explicație');
            else if(el.className.includes('ai')) el.setAttribute('aria-label','AI Chat');
            else el.setAttribute('aria-label','Buton');
          }
          el.addEventListener('keydown',e=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); el.click(); }});
        }
      };
      sels.forEach(sel=> root.querySelectorAll(sel).forEach(force));
    })();

    // 3) A11y + aria-expanded sincron pe accordion
    (function ensureAccordionA11y(){
      const acc=root.querySelector('.custom-accordion, .explain-accordion'); if(!acc) return;
      const header=acc.querySelector('.custom-accordion-header, .accordion-header');
      const content=acc.querySelector('.custom-accordion-content, .accordion-content');
      if(!header||!content) return;
      header.setAttribute('role','button'); header.setAttribute('tabindex','0');
      if(!content.id) content.id='acc-content-'+Math.random().toString(36).slice(2);
      header.setAttribute('aria-controls',content.id);
      const openNow=acc.classList.contains('open')||(getComputedStyle(content).maxHeight&&getComputedStyle(content).maxHeight!=='0px');
      header.setAttribute('aria-expanded',openNow?'true':'false');
      const mo=new MutationObserver(()=>{ const open=acc.classList.contains('open'); header.setAttribute('aria-expanded',open?'true':'false'); });
    ...
    // 4) Sanitizare „din afară”: interceptăm doar inserările în zonele de Q/A/Explanation
    (function patchInsertAdjacentHTML(){
      ...
      Element.prototype.insertAdjacentHTML=function(position,html){
        try{ const must=this.closest&&this.closest(SANITIZE_SEL); if(must){ return orig.call(this,position,safeTextHTML(html)); } }catch(_){}
        return orig.call(this,position,html);
      };
    })();

    // 5) Aliasuri tolerante (inofensive dacă structura e deja corectă)
    (function aliases(){
      const aliasMap=[
        ['.option-card-exam','option-card'],
        ['.option-item-exam','option-item'],
        ['.question-title','question-text'],
        ['.question_richtext','question-text'],
        ['.question-counter','progress-counter'],
        ['.progress__counter','progress-counter'],
        ['.explanation-card','explanation-btn'],
        ['.question-item','question-wrapper']
      ];
      aliasMap.forEach(([fromSel,addClass])=>{
        root.querySelectorAll(fromSel).forEach(el=> el.classList.add(addClass));
      });
      root.querySelectorAll('[data-option-card]').forEach(el=> el.classList.add('option-card'));
      root.querySelectorAll('[data-option-item]').forEach(el=> el.classList.add('option-item'));
    })();
  });
})();
/* ====== /MINI-UTIL ====== */

/* ===================== MAIN SCRIPT (A/B tolerant) ===================== */
...
/* ===== A/B wrapper detection =====
   - Suportă:
     a) Single:  .question-wrapper (clasic)
     b) Double:  .question-wrapper-A + .question-wrapper-B (A/B buffer)
*/
const QSEC=$(".question-section");
const WRA=$(".question-wrapper-A");
const WRB=$(".question-wrapper-B");
const IS_AB = !!(WRA && WRB);
const SINGLE_WRAP = !IS_AB ? ($(".question-wrapper") || (QSEC && QSEC.querySelector(".question-wrapper"))) : null;

/* Marcare pentru cross-fade */
if (IS_AB){ WRA.classList.add("q-fade"); WRB.classList.add("q-fade"); }
else if (SINGLE_WRAP){ SINGLE_WRAP.classList.add("q-fade"); }
...
let WRONG_PREPARED=[],WRONG_PREPARED_READY=false,START_WRONG_IN_PROGRESS=false; /* <-- definit O SINGURĂ DATĂ */
...
async function prefetchMany(indices,priorityMap={}){
  ...
  try{
    const chunk=list.slice(0,10);
    const qs=await db.collection("mediu_invatare_pages").where("chapterSlug","==",CHAP).where("Index","in",chunk).get();
    qs.forEach(doc=>{
...
function revealNow(){
  ...
  blurActive();
  if(QSEC) QSEC.style.setProperty("display","flex","important");
  if (IS_AB){ (WRA||$(".question-wrapper-A"))?.classList.add("visible"); }
  else { (SINGLE_WRAP||$(".question-wrapper"))?.classList.add("visible"); }
  const loader=$(".lesson-loader"); if(loader) loader.style.setProperty("display","none","important");
}

/* ===== AI CHAT label forțat ===== */
...
function applyAIChatText(){ forceAIChatText(refs.aiBtn); forceAIChatText(refs.aiBtnMobile); }
...
/* ===== Refs struct (desktop + mobil) – per wrapper ===== */
const refs={
  /* active scope */
  scope:null,
  /* common */
  ...
  /* mobile (global, poate fi în afara wrapperelor) */
...
/* ===== Hydrate pentru un scope dat (single sau A/B) ===== */
function hydrateInto(scopeRoot){
  const root = scopeRoot || (QSEC||D);
  const need=(sel)=>root.querySelector(sel);
  refs.scope=scopeRoot||root;
  ...
  refs.progress = (QSEC||D).querySelector(".progress-counter"); /* progress poate fi global */
  refs.mainWrap = need(".main-content-wrapper");
  refs.optWrap = need(".options-wrapper");
  refs.cards = a(".option-card",refs.optWrap||refs.qWrap||root);
  ...
  /* mobile out-of-scope (că e layout separat) */
  const mobileRoot=(QSEC||D);
  refs.buttonsWrapMobile = mobileRoot.querySelector(".submit-next-buttons-mobile")||null;
  if(refs.buttonsWrapMobile){
    ...
    refs.submitBtnMobile=refs.explainBtnMobile=refs.aiBtnMobile=null;
  }
  ...
  refs.feedbackWrap = (QSEC||D).querySelector(".feedback-wrapper"); /* feedback poate fi comun */
  ...
  /* sanity checks */
  if(!refs.qText || !refs.progress || !refs.mainWrap || !refs.optWrap || !refs.cards.length || !refs.buttonsWrap || !refs.submitBtn || !refs.feedbackWrap){
    return false;
  }

  /* Forțează eticheta AI CHAT (desktop+mobil) */
  applyAIChatText();
  ...
  const onExplainClick=()=>{
    ...
    if(opening){
      ...
      if(refs.accArrow){ try{ refs.accArrow.style.removeProperty("transform"); }catch(_){ } }
      setExplainBtnUI(false);
      scrollContainerToAnchor(refs.anchorTop,scroller,{behavior:"smooth"});
    }
  };
  if(refs.explainBtn && !refs.explainBtn.dataset.boundExp){ refs.explainBtn.addEventListener("click", onExplainClick); refs.explainBtn.dataset.boundExp='1'; }
  if(refs.explainBtnMobile && !refs.explainBtnMobile.dataset.boundExp){ refs.explainBtnMobile.addEventListener("click", onExplainClick); refs.explainBtnMobile.dataset.boundExp='1'; }
  ...
  if(refs.aiBtn && !refs.aiBtn.dataset.boundAi){ refs.aiBtn.addEventListener("click", onAiClick); refs.aiBtn.dataset.boundAi='1'; }
  if(refs.aiBtnMobile && !refs.aiBtnMobile.dataset.boundAi){ refs.aiBtnMobile.addEventListener("click", onAiClick); refs.aiBtnMobile.dataset.boundAi='1'; }
...
const AB = {
  active: IS_AB ? "A" : "S",
  get current(){ return IS_AB ? (this.active==="A"?WRA:WRB) : (SINGLE_WRAP); },
  get other(){ return IS_AB ? (this.active==="A"?WRB:WRA) : null; },
  swap(){ if(IS_AB) this.active = (this.active==="A"?"B":"A"); }
};

/* ===== Buttons locks & layout toggles ===== */
...
function ensureAIPop(){
  ...
  if(!AIPop){
    ...
    const audioBtn=document.createElement("button"); audioBtn.className="audio-ai-btn"; audioBtn.textContent="Audio AI"; Object.assign(audioBtn.style,{padding:"10px 14px",borderRadius:"10px",border:"1px solid #3a2a2a",background:"#1d1d1d",color:"#fff",cursor:"pointer"});
...
/* ===== Hydrate inițial (single sau A/B: active buffer) ===== */
if (!hydrateInto(AB.current||SINGLE_WRAP)){
  alert("Structura nu e completă. Verifică clasele HTML.");
  document.body.classList.remove("page-loading");
  return;
}

/* ===== INIT progres & reluare wrong-mode ===== */
...
(function(){ const o=$(".chapter-progress-bar-outer"); if(!o) return;
  if(!$(".chapter-final-screen .progress-green")){ const s=document.createElement("span"); s.className="progress-green"; o.appendChild(s); }
  if(!$(".chapter-final-screen .progress-red"))  { const s=document.createElement("span"); s.className="progress-red";   o.appendChild(s); }
  if(!$(".chapter-final-screen .progress-gap"))  { const s=document.createElement("span"); s.className="progress-gap";   o.appendChild(s); }
})();

/* ===== Failsafe final loader off (folosit și la reveal) ===== */
document.body.classList.remove("page-loading");
blurActive();

/* ===== START: prefetch + reveal inițial ===== */
...
/* ===== Render/Update (single + A/B) ===== */
async function updateInPlace(idx,{resetSelections=false}={}){
  setButtonsTransitionLock(true);
  const q=BUF[idx];
  if(!q){ await showFinal(); setButtonsTransitionLock(false); return; }

  /* Alege wrapperul de stage (A/B) sau single */
  const stage = IS_AB ? AB.other : AB.current;
  const active = IS_AB ? AB.current : AB.current;

  /* Hydrate pe stage ca refs* să țintească exact DOM-ul în care scriem */
  if(!hydrateInto(stage)){
    alert("Structura nu e completă în wrapperul curent. Verifică clasele HTML.");
    setButtonsTransitionLock(false);
    return;
  }

  /* Pregătim înălțimea pentru a evita sărituri vizibile */
  const container = refs.qWrap || stage;
  const prevH = (active||container).offsetHeight;
  container.classList.add("no-transition","is-preparing");
  if(prevH) container.style.minHeight = prevH + "px";

  /* Titlu capitol + progres */
  (document.querySelector(".chapter-text")||document.createElement("div")).textContent = sanitizeMaybe(chapterName);
  if(refs.progress){
  ...
  if(refs.qText){
    refs.qText.innerHTML = "";
    refs.qText.insertAdjacentHTML("afterbegin", sanitizeMaybe(q.Question||""));
  }
  ...
  /* Reset UI: ascunde feedback + explain/AI până după verificare */
  ...
  if(refs.aiBtnMobile){ try{ refs.aiBtnMobile.style.display="none"; }catch(_){} }
  applyAIChatText();

  /* Închide accordion/video */
  ...
  /* La început arătăm un singur buton pe desktop; pe mobil nu schimbăm layout-ul Webflow */
  setSingleBtnState(true);
  setSubmitStateBoth("verify");

  /* Imagine (decodare înainte de swap pentru tranziție curată) */
  ...
  /* Restaurare răspuns existent (atenție la sesiune wrong-mode) */
  try{
    ...
    if(snap.exists){
      ...
      if(allowRestore && selSaved.length>0){
        ...
        if(refs.correctAns){ const ord=["A","B","C"].filter(x=>corSaved.includes(x)); refs.correctAns.textContent=ord.join(", "); }
        ...
        if(refs.aiBtnMobile){ try{ refs.aiBtnMobile.style.setProperty("display","flex","important"); }catch(_){} }
        applyAIChatText();
  ...
  /* Cross-fade (A/B) sau simplu (single) */
  if(IS_AB){
    const prev = AB.current, next = stage;
    next.classList.add("visible");
    prev.classList.remove("visible");
    AB.swap();
    hydrateInto(AB.current);
  }
...
/* ===== Final screen + pre-pregătire WRONG ===== */
async function showFinal(){
  ...
  /* Afișează butonul „Refă doar greșite” doar dacă există greșite */
  if(resetWrongBtn){
    resetWrongBtn.style.display="none";
    let hasWrong=false;
    answersSnap.forEach(doc=>{
      const d=doc.data();
      const sel=(d.selected||[]).slice().sort().join(",");
      const cor=(d.correct||[]).slice().sort().join(",");
  ...
  try{
    ...
    WRONG_PREPARED=[]; WRONG_PREPARED_READY=true;
  }
}

/* ===== Build wrong list acum (fallback) ===== */
...
/* ===== Wrong-mode: start safe (cu persistență) ===== */
async function startWrongModeSafe(){
  ...
  const actWrap = AB.current || document.querySelector(".question-wrapper") || document.body;

  /* Îngheață UI */
  if(actWrap){
    const h=actWrap.offsetHeight;
    actWrap.classList.add("no-transition","is-preparing");
    if(h) actWrap.style.minHeight=h+"px";
  }
  ...
  try{
    ...
    if(!list.length){
      ...
      FINAL?.style.setProperty("display","flex","important");
      if(actWrap){ actWrap.classList.remove("no-transition","is-preparing"); actWrap.style.minHeight=""; }
      document.body.classList.remove("page-loading");
    ...
    const q1=BUF[list[0]]; try{ await ensureImageReadyFromQ(q1); }catch(_){}
    ...
    if(loaderEl) loaderEl.style.setProperty("display","none","important");
    document.body.classList.remove("page-loading");
    const actWrap2 = AB.current || document.querySelector(".question-wrapper") || document.body;
    if(actWrap2){ actWrap2.classList.remove("no-transition","is-preparing"); actWrap2.style.minHeight=""; }
    START_WRONG_IN_PROGRESS=false;
  }
}

/* ===== Next chapter pre-calc cache ===== */
...
/* ===== Submit / Next / Finish (delegare, desktop + mobil) ===== */
(QSEC||document).addEventListener("click",async(e)=>{
  ...
  if(isVerify){
    ...
    const isFullyCorrect=selected.length>0&&selected===correct;
    ...
    if(refs.correctAns){ const ord=["A","B","C"].filter(x=>cor.includes(x)); refs.correctAns.textContent=ord.join(", "); }
    ...
    /* Vizibilizează explicație + AI (desktop + mobil) */
    if(refs.feedbackWrap) refs.feedbackWrap.style.setProperty("display","flex","important");
    if(refs.explainBtn) refs.explainBtn.style.setProperty("display","flex","important");
    if(refs.explainBtnMobile) refs.explainBtnMobile.style.setProperty("display","flex","important");
    if(refs.aiBtn) refs.aiBtn.style.setProperty("display","flex","important");
...
/* ===== Initial render efectiv (după prefetch/reveal din Partea 1) ===== */
(async function initialRender(){
  if(WRMODE && Array.isArray(WRONG) && WRONG.length){
    await updateInPlace(WRONG[WRP],{resetSelections:true});
    await prefetchAhead(WRONG[WRP]||0);
  }else if(!FIN){
    await updateInPlace(CUR,{resetSelections:false});
    await prefetchAhead(CUR||0);
  }
})();
