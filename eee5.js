...
const ACTIVE="is-active";
const getActiveWrap=()=> (WA.classList.contains(ACTIVE)?WA:WB);
const getBufferWrap=()=> (WA.classList.contains(ACTIVE)?WB:WA);
...
/* ===== Final screen progress bar helpers ===== */
...
/* ===== UI helpers ===== */
function revealNow(){ D.body.classList.remove("page-loading"); blurActive(); if(QSEC) QSEC.style.setProperty("display","flex","important"); makeVisible(getActiveWrap()); const loader=$(".lesson-loader"); if(loader) loader.style.setProperty("display","none","important"); }
...
/* ===== Render (with robust reset & flicker guard) ===== */
async function renderQuestionInWrapper(refs,idx,{resetSelections=false}={}){
  // Resetăm întotdeauna interacțiunea; dacă există răspuns salvat, va fi rehidratat mai jos.
  ANSW=false; SEL=[];
...
async function flipToIndex(idx,{resetSelections=false}={}){ const buf=bufferWrap(), bufRefs=refsOf(buf); await renderQuestionInWrapper(bufRefs,idx,{resetSelections}); const act=activeWrap(); act.classList.remove(ACTIVE); buf.classList.add(ACTIVE); updateRefsAfterFlip(); act.classList.remove("visible"); buf.classList.add("visible"); }
...
if(FIN){
  ...
  await renderQuestionInWrapper(REFS, CUR, {resetSelections:true});
  revealNow();
}
  ...
  if(isNext && ANSW){
    if(WRMODE){
      ...
      if(CUR<TOTAL){
        ...
        // 🔧 IMPORTANT: resetSelections=true pentru a permite click pe opțiunile următoarei întrebări
...
D.addEventListener("keydown",e=>{
  const act=getActiveWrap(), refs=(act===WA)?REFS_A:REFS_B;
